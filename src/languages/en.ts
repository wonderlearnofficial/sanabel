const en = {
  translation: {
    "todo.page.title": "Task List",
    "todo.search": "Search",
    "todo.searchPlaceholder": "Search for a mission or category",
    "todo.date.today": "Today",
    "todo.date.yesterday": "Yesterday",
    "todo.date.select": "Select date",
    "todo.date.previous": "Previous day",
    "todo.date.next": "Next day",
    "todo.date.previousMonth": "Previous month",
    "todo.date.nextMonth": "Next month",
    "todo.date.backToToday": "Back to today",
    "todo.date.firstDay": "This is your first account day; there is no earlier mission history.",
    "todo.date.noEarlierHistory": "There is no earlier Solo mission history.",
    "todo.date.cancel": "Cancel",
    "todo.date.done": "Done",
    "todo.status.label": "Status",
    "todo.status.all": "All",
    "todo.status.inProgress": "In Progress",
    "todo.status.pending": "Pending",
    "todo.status.completed": "Completed",
    "todo.status.notCompleted": "Not completed",
    "todo.status.denied": "Denied",
    "todo.source.label": "Source",
    "todo.source.all": "All",
    "todo.source.self": "Added by me",
    "todo.source.teacher": "Teacher",
    "todo.source.parent": "Parent",
    "todo.source.multiple": "Multiple sources",
    "todo.sort.label": "Sort",
    "todo.sort.manual": "My order",
    "todo.sort.newest": "Newest",
    "todo.sort.oldest": "Oldest",
    "todo.sort.bySource": "By source",
    "todo.approval.waiting": "Waiting for approval",
    "todo.approval.waitingForPrefix": "Waiting for approval from",
    "todo.approval.approvedBy": "Approved by",
    "todo.approval.request": "Request approval",
    "todo.approval.retarget": "Request approval from someone else",
    "todo.approval.cancel": "Cancel request",
    "todo.assignment.assignedBy": "Assigned by",
    "todo.assignment.details": "Assignment details",
    "todo.details": "Details",
    "todo.sources.view": "View sources",
    "todo.remove": "Remove from my list",
    "todo.addMission": "Add New Mission",
    "todo.empty.all": "No missions",
    "todo.empty.inProgress": "No missions in progress",
    "todo.empty.pending": "No pending missions",
    "todo.empty.completed": "No completed missions",
    "todo.empty.results": "No results",
    "todo.retry": "Retry",
    "todo.loading": "Loading...",
    "todo.loadError": "Could not load missions. Check your connection.",
    "todo.historical.pending": "You have {{count}} pending requests from previous days",
    "todo.historical.missionNotCompleted": "Mission not completed",
    "todo.historical.completedLater": "Completed later",
    "todo.historical.missionDate": "Mission date",
    "todo.historical.approvalDate": "Approval date",
    "todo.reorder": "Reorder",
    "todo.reorder.done": "Done",
    "بانتظار": "Waiting for",
    "todo.multiSourceLine": "{{count}} sources · last assigned {{date}}",
    "ابحث عن مهمة أو تصنيف": "Search for a mission or category",
    "تعذر حفظ ترتيب المهام": "Could not save the mission order",
    "تعذر تحميل المهام. تحقق من اتصالك.": "Could not load missions. Check your connection.",
    "الحالة": "Status",
    "خيارات المهمة": "Mission options",
    "عرض التفاصيل": "View details",
    "إزالة من قائمتي": "Remove from my list",
    "إزالة المهمة؟": "Remove this mission?",
    "سيتم حذفها من قائمة مهامك فقط.": "It will only be removed from your list.",
    "طلب الموافقة من شخص آخر": "Ask someone else to approve",
    "بانتظار موافقة": "Waiting for approval from",
    "أضفتها في": "Added on",
    "أنت": "You",
    "تفاصيل المهمة": "Mission details",
    "مصادر المهمة": "Mission sources",
    "المصدر": "Source",
    "أضفتها أنا": "Added by me",
    "الترتيب": "Sort",
    "ترتيبي": "My order",
    "حسب المصدر": "By source",
    "من المعلمين": "From teachers",
    "من أولياء الأمور": "From parents",
    "تم إرسال الطلب إلى": "Request sent to",
    "اسحب لإعادة الترتيب": "Drag to reorder",
    "لا توجد نتائج مطابقة": "No matching results",
    "لا توجد مهام قيد التنفيذ": "No missions in progress",
    "أضفتها أنت": "Added by you",
    "المعلم": "Teacher",
    "مصادر متعددة": "Multiple sources",
    "todo.multiSource": "Assigned from {{count}} sources",
    "هيا بنا نصنع الخير معًا!": "Let's do good together!",
    "تقدم حسناتك": "Your good-deed progress",
    "سنبلة الصيام": "Fasting Sanabel",
    "سنبلة الصدقة": "Charity Sanabel",
    "سنبلة الصلاة": "Prayer Sanabel",
    "تغيير اللغة": "Change language",
    "حذف المهمة": "Delete mission",
    "تعذر حفظ جلسة تسجيل الدخول على هذا الجهاز":
      "The login session could not be saved on this device.",
    "حفظ الحساب": "Save account",
    "اسم والدك": "Parent's name",
    "اختر مرحلتك الدراسية 🎓": "Choose your school stage 🎓",
    "الابتدائي": "Primary",
    "الإعدادي": "Preparatory",
    "الثانوي": "Secondary",
    "أدخل رمزاً لولي أمرك": "Enter your parent's code",
    "رمز الوالدين": "Parent code",
    "اصنع رمزاً لولي أمرك حتى يمكنهم متابعة تقدمك":
      "Create a code so your parent can follow your progress",
    "ضع صورة شخصية لك": "Choose your avatar",
    "ابدأ رحلة جمع الحسنات": "Start your good-deed journey",
    fill_all_fields: "Please fill in all fields",
    invalid_email: "Enter a valid email address",
    short_password: "Password is too short",
    login_successful: "Login successful",
    passwordNotMeetRequirements: "Password does not meet the requirements",
    passwordChangeFailed: "Password change failed",
    emptyFieldError: "This field is required",
    enter_birthdate: "Enter your birth date",
    complete_account: "Complete account",
    year: "Year",
    month: "Month",
    day: "Day",
    continue: "Continue",
    year_out_of_range: "Year is out of range",
    four_digit_year: "Enter a four-digit year",
    ERROR: "Error",
    pleaseSelectRole: "Please select a role",
    pleaseSelectAvatar: "Please select an avatar",
    Male: "Male",
    Female: "Female",
    حسنة: "Good deed",
    "حذف الحساب": "Delete account",
    alreadyPassed: "Already passed",
    timeLeft: "Time left",
    School: "School",
    Company: "Company",
    Charity: "Charity",
    "لا يوجد اتصال بالإنترنت": "No internet connection",
    "يبدو أن اتصالك بالإنترنت منقطع": "It looks like your internet connection is offline",
    "تأكد من اتصالك وحاول مرة أخرى": "Check your connection and try again",
    "نصائح للحل:": "Troubleshooting tips:",
    "تحقق من اتصال الواي فاي": "Check your Wi-Fi connection",
    "أعد تشغيل جهاز التوجيه": "Restart your router",
    "تحقق من البيانات المحمولة": "Check your mobile data",
    "اهداء من وقف د.حسن العديسي": "A gift from Dr. Hassan Al-Adisi Endowment",
    "Powered By": "Powered by",
    "Personalized Learning Experience": "Personalized Learning Experience",
    Character: "Character",
    Customization: "Customization",
    "Personalized avatars with customizable hairstyles, clothing colors, skin tones, and backgrounds.":
      "Personalized avatars with customizable hairstyles, clothing colors, skin tones, and backgrounds.",
    "Sanabel Al Ihsan": "Sanabel Al Ihsan",
    "admin.chart.distributionAndAnalytics": "Distribution & Analytics",
    "admin.header.description.scores": "Monitor student scores, XP, level, medal, tree progress, and Sanabel balances.",
    "admin.header.description.history": "View recent completed student missions and awarded rewards.",
    "admin.header.description.users": "Manage every user on your platform.",
    "admin.header.description.students": "Monitor student records and account details.",
    "admin.header.description.teachers": "Assign and manage teachers' classes.",
    "admin.header.description.parents": "Review parent connection codes and family links.",
    "admin.header.description.admins": "Manage administrator access.",
    "admin.header.description.classes": "Create and manage classes.",
    "admin.header.description.organizations": "Configure schools and organizations.",
    "admin.header.description.grades": "Manage educational grades and academic tracks.",
    "admin.noData": "No data",
    "admin.chart.overviewSummary": "Overview summary",
    "مفعّل": "Enabled",
    "معطّل": "Disabled",
    "لاحقاً": "Later",
    "جاري تحميل البيانات...": "Loading data...",
    "admin.saving": "Saving...",
    "admin.saveChanges": "Save changes",
    "admin.active": "Active",
    "الخطوة": "Step",
    "من": "of",
    Yes: "Yes",
    No: "No",
    "Search users, emails, IDs...": "Search users, emails, IDs...",
    "Grade: All": "Grade: All",
    "School: All": "School: All",
    "Role: All": "Role: All",
    "Verified Only": "Verified only",
    "Clear Filters": "Clear filters",
    Showing: "Showing",
    of: "of",
    results: "results",
    People: "People",
    Academics: "Academics",
    "تعذر تحميل قائمة المدارس": "Could not load the school list",
    "حذف المدرسة": "Delete school",
    "الاسم الأول والبريد الإلكتروني مطلوبان": "First name and email are required",
    "نحن نعمل بجد لتوفير تجربة مظلمة مريحة للعينين. ترقبوا":
      "We are working hard on a comfortable dark-mode experience. Stay tuned.",
    passwordChangeSuccess: "Password changed successfully",
    "الأشعارات": "Notifications",
    "لا يوجد إشعارات!": "No notifications!",
    "لم تتلقَ أي إشعار حتى الآن": "You have not received any notifications yet",
    "حدث خطأ أثناء التسجيل": "Registration failed",
    "خطأ في السيرفر: ": "Server error: ",
    "خطأ في الشبكة": "Network error",
    "حدث خطأ غير متوقع": "An unexpected error occurred",
    enterFirstName: "Enter your first name",
    enterLastName: "Enter your last name",
    invalidFirstName: "Enter a valid first name",
    invalidLastName: "Enter a valid last name",
    errorOccurred: "An error occurred",
    "جاري الإنشاء": "Creating...",
    "يرجى اختيار صورة شخصية قبل المتابعة": "Choose an avatar before continuing",
    "حدث خطأ، حاول مرة أخرى": "Something went wrong. Try again.",
    "لا توجد طلبات موافقة حاليًا": "There are no approval requests right now",
    "ستظهر هنا طلبات إنجاز المهام التي يرسلها أبناؤك أو طلابك":
      "Mission requests from your children or students will appear here",
    "موافقة": "Approve",
    "رفض": "Deny",
    "منذ {{m}} دقيقة": "{{m}} minutes ago",
    "منذ {{h}} ساعة": "{{h}} hours ago",
    "أمس": "Yesterday",
    "حصلت على كأس جديد!": "You earned a new trophy!",
    "حدث خطأ أثناء إرسال طلب الموافقة": "Could not send the approval request",
    "اربط ولي أمر لطلب الموافقة": "Link a parent to request approval",
    "بانتظار الموافقة...": "Waiting for approval...",
    "إعادة الطلب": "Request again",
    "طلب الموافقة": "Request approval",
    "إرسال طلب الموافقة": "Send approval request",
    "اختر ولي الأمر أو المعلم للموافقة:": "Choose a parent or teacher to approve:",
    "اختر...": "Choose...",
    "ولي الأمر": "Parent",
    "تم إرسال الطلب!": "Request sent!",
    "بانتظار موافقة ولي أمرك أو معلمك": "Waiting for your parent or teacher to approve",
    "حسنًا": "OK",
    "جارٍ تحميل الفصول...": "Loading classes...",
    howToResetPassword: "How do I reset my password?",
    resetPasswordAnswer: "Use Forgot Password on the login screen and follow the verification steps.",
    howToCreateAccount: "How do I create an account?",
    createAccountAnswer: "Choose Create Account and complete the registration steps.",
    howToUpdateProfile: "How do I update my profile?",
    updateProfileAnswer: "Open Profile, choose Edit, then save your changes.",
    howToContactSupport: "How do I contact support?",
    contactSupportAnswer: "Use the support contact shown in the Help Center.",
    howToReportIssue: "How do I report an issue?",
    reportIssueAnswer: "Send support the exact error and the steps that caused it.",
    howToDeleteAccount: "How do I delete my account?",
    deleteAccountAnswer: "Open Profile settings and choose Delete account.",
    privacyAndSecurity: "Privacy and security",
    privacySecurityAnswer: "Your account data is protected and used to provide Sanabel services.",
    troubleshooting: "Troubleshooting",
    troubleshootingAnswer: "Check your connection, retry, then contact support with the exact error.",
    helpCenter: "Help center",
    helpCenterDescription: "Answers to common questions about your Sanabel account.",
    "جاري تحميل النشاطات...": "Loading activities...",
    loading: "Loading...",
    Level: "Level",
    "لا توجد بيانات لعرضها": "There is no data to display",
    "لا توجد جوائز متاحة حالياً": "No trophies are currently available",
    "مرحباً يا أبطال!": "Welcome, heroes!",
    "اختروا مغأمرتكم القادمة": "Choose your next adventure",
    "تحديات الإحسان": "Good-deed challenges",
    "أم": "or",
    "سنابل الخير؟": "Sanabel of goodness?",
    "تحديات ": "Challenges ",
    "سنابل ": "Sanabel ",
    "ترقية مستوى!": "Level up!",
    "ترتيبك": "Your rank",
    "لا توجد فئات مهام متاحة حاليًا": "No mission categories are currently available",
    "تعذر تحميل فئات المهام. حاول مرة أخرى.": "Could not load mission categories. Try again.",
    "لا توجد أنواع مهام متاحة في هذه الفئة": "No mission types are available in this category",
    "تعذر تحميل أنواع المهام. حاول مرة أخرى.": "Could not load mission types. Try again.",
    "لا توجد مهام متاحة لهذا النوع": "No missions are available for this type",
    "تعذر تحميل المهام. حاول مرة أخرى.": "Could not load missions. Try again.",
    "العودة للفئات": "Back to categories",
    "العودة للأنواع": "Back to types",
    "فصل كبير (+20 طالب)": "Large class (20+ students)",
    "فصل صغير (-10 طلاب)": "Small class (under 10 students)",
    "عدد الطلاب (كثير-قليل)": "Student count (high to low)",
    "المستوى (عالي-منخفض)": "Level (high to low)",
    "فصل 2/1 الابتدائي": "Primary class 1/2",
    "طلاب الفصل️": "Class students",
    "تسجيل حسنات": "Register good deeds",
    "اختر صفك": "Choose your grade",
    "تغيير كلمة السر": "Change password",
    "كلمة السر الجديدة": "New password",
    "ساعد الطلاب في جمع الحسنات ": "Help students collect good deeds ",
    "دعوة طالب": "Invite student",
    "لا يوجد فصل": "No class",
    "لم يتم اختيار أي طالب": "No students selected",
    "لوحة المتصدرين تكتمل عندما يشارك ثلاثة طلاب على الأقل":
      "The leaderboard fills up once at least three students take part",
    "مكان شاغر": "Open spot",
    "لا يوجد ولي أمر أو معلم مرتبط بحسابك":
      "No parent or teacher is linked to your account",
    "لا يوجد طالب بهذا الاسم": "No student matches that name",
    "لا يوجد صف": "No grade",
    "حذف المهمة؟": "Delete this mission?",
    "جاري الحذف...": "Deleting...",
    "اختر الفئة المناسبة للمهمة": "Pick the category that fits the mission",
    "اختر فئة المهمة": "Choose a mission category",
    "حدد نوع السلوك أو القيمة التي تريد تعزيزها":
      "Choose the behaviour or value you want to encourage",
    "الطلاب المختارون": "Selected students",
    "تغيير الطلاب": "Change students",
    "نوع الإجراء": "Action type",
    "ستظهر في قائمة مهام الطالب": "Added to the student's mission list",
    "تسجيل أن الطالب أنجزها": "Records that the student has done it",
    "متابعة لاختيار المهمة": "Continue to choose a mission",
    "متابعة لتسجيل المهمة": "Continue to register the mission",
    "اختر فئة للمتابعة": "Choose a category to continue",
    "خطوات إسناد المهمة": "Mission assignment steps",
    // Step labels are namespaced: the bare word النوع already means "Gender"
    // elsewhere in this catalogue.
    "mission.step.students": "Students",
    "mission.step.category": "Category",
    "mission.step.type": "Type",
    "mission.step.mission": "Mission",
    "students.selectedOfTotal": "{{selected}} of {{total}} selected",
    "students.selectedCount": "{{count}} students selected",
    "mission.assign.created": "{{count}} missions assigned",
    "mission.assign.existing": "{{count}} already active",
    "mission.assign.completedToday": "{{count}} already completed today",
    "mission.complete.completed": "{{count}} registered",
    "mission.complete.already": "{{count}} already completed",
    "mission.complete.failed": "{{count}} failed",
    "إزالة": "Remove",
    "الإنجازات": "Achievements",
    "قائمة المتصدرين": "Leaderboard",
    "تفاصيل الفريق": "Team details",
    "فريق الخير": "Goodness team",
    "100 طالب": "100 students",
    "Monitor student scores, XP, level, medal, tree progress, and sanabel counts.":
      "Monitor student scores, XP, level, medal, tree progress, and sanabel counts.",
    "View recent student task completion history and rewards granted.":
      "View recent student task completion history and rewards granted.",
    "Manage every user inside your platform.": "Manage every user inside your platform.",
    "Monitor student records and credentials.": "Monitor student records and credentials.",
    "Assign and manage classrooms for teachers.": "Assign and manage classrooms for teachers.",
    "Review parent link codes and family links.": "Review parent link codes and family links.",
    "Manage system administrator level access.": "Manage system administrator level access.",
    "Create and modify classes and grades.": "Create and modify classes and grades.",
    "Configure schools and organizations.": "Configure schools and organizations.",
    "Manage learning grades and academic streams.": "Manage learning grades and academic streams.",
    "admin.treeStage": "Stage",
    "admin.completed": "Completed",
    "admin.deleteItemsConfirm": "Delete {{count}} items? This cannot be undone.",
    "admin.resetPasswordFor": "Reset password for {{name}}?",
    "المرحلة غير محددة": "Stage not specified",
    "متوسط الميداليات": "Average medals",
    "{{count}} حسنة": "{{count}} good deeds",
    "تسجيل دخول سريع (وضع التطوير)": "Quick login (development mode)",
    "اختر حسابًا لتسجيل الدخول به مباشرة بدون كلمة مرور — متاح فقط أثناء التطوير المحلي.":
      "Choose an account to sign in directly without a password—available only during local development.",
    "فتح التطبيق بشكل طبيعي": "Open the app normally",
    "تعذر تحميل المستخدمين. تأكد أن السيرفر يعمل محليًا (NODE_ENV غير production).":
      "Could not load users. Make sure the server is running locally with NODE_ENV not set to production.",
    "تعذر تحميل قائمة المستخدمين — تأكد أن السيرفر يعمل في وضع التطوير":
      "Could not load the user list—make sure the server is running in development mode.",
    "تعذر تسجيل الدخول بهذا الحساب": "Could not sign in with this account",
    "لا يوجد حسابات": "No accounts",
    "دخول": "Sign in",
    "منذ دقيقة": "One minute ago",
    "دعوة لمتابعة التقدم في الحسنات": "An invitation to follow good-deed progress",
    "admin.panelName": "Sanabel Admin",
    "admin.nav.group.analytics": "Analytics",
    "admin.nav.group.people": "User management",
    "admin.nav.group.activity": "Activity & progress",
    "admin.nav.group.academics": "Academic data",
    "admin.nav.group.system": "System",
    "admin.shell.collapse": "Collapse menu",
    "admin.shell.expand": "Expand menu",
    "admin.shell.pageHelp": "Help for this page",
    "admin.shell.logout": "Sign out",
    "admin.shell.openNavigation": "Open navigation",
    "admin.shell.breadcrumb": "Breadcrumb",
    "admin.shell.data": "Data",
    "admin.shell.accountMenu": "Account menu",
    "admin.shell.loading": "Loading...",
    "admin.search.globalPlaceholder": "Search users, schools and classes...",
    "admin.search.noResults": "No matching results",
    "admin.export.hint": "Export the current page to CSV",
    "admin.import.unsupportedHere": "Import is not available on this page",
    "admin.scope.superAdmin": "Super Admin",
    "admin.scope.schoolAdmin": "School Admin",
    "admin.analytics.level": "Level",
    "Data copied to clipboard!": "Data copied to clipboard!",
    "Sanabel Data Extractor": "Sanabel Data Extractor",
    "This component extracts all categories, sanabel types, and missions for translation.":
      "This component extracts all categories, sanabel types, and missions for translation.",
    "Extracting...": "Extracting...",
    "Extract All Data": "Extract all data",
    "Summary:": "Summary:",
    "Total Categories:": "Total categories:",
    "Total Sanabel Types:": "Total Sanabel types:",
    "Total Missions:": "Total missions:",
    "Total Translation Items:": "Total translation items:",
    "Download Full JSON": "Download full JSON",
    "Copy JSON to Clipboard": "Copy JSON to clipboard",
    "Translation Array:": "Translation array:",
    "Here's a simple array of all text that needs translation:": "Here is an array of all text requiring translation:",
    "Copy Translation Array": "Copy translation array",
    "Extracted Data Structure:": "Extracted data structure:",
    Category: "Category",
    "ID:": "ID:",
    "Sanabel Type:": "Sanabel type:",
    "Missions:": "Missions:",
    Mission: "Mission",
    "View Raw JSON Data": "View raw JSON data",
    "Sanabel — Simulation Dashboard": "Sanabel — Simulation Dashboard",
    "Balance calculator · {{count}} total challenge entries": "Balance calculator · {{count}} total challenge entries",
    "Input Total XP": "Input total XP",
    "Current Medal": "Current medal",
    "Unlocked at level {{level}}": "Unlocked at level {{level}}",
    "XP Formula": "XP formula",
    "Medal Milestones": "Medal milestones",
    "Complete Challenges": "Complete challenges",
    Complete: "Complete",
    History: "History",
    "Reset all": "Reset all",
    "Current Inventory": "Current inventory",
    "Level from Inventory XP": "Level from inventory XP",
    "XP in current level": "XP in current level",
    "Reward Rates per Type": "Reward rates per type",
    "XP & Levels": "XP & levels",
    "Inventory Sim": "Inventory simulation",
    "Tree Preview": "Tree preview",
    "Level Table": "Level table",
    "Blue Snabel": "Blue Sanabel",
    "Red Snabel": "Red Sanabel",
    "Yellow Snabel": "Yellow Sanabel",
    الرئيسية: "Home",
    التحديات: "Challenges",
    المتصدرين: "Leaderboard",
    استعراض: "Browse",
    الإعدادات: "Settings",
    التقدم: "Progress",
    البروفايل: "Profile",
    سماد: "Fertilizer",
    ماء: "Water",
    "تعذر تحميل بيانات الحساب": "We couldn't load your account data",
    "تحقق من اتصالك ثم حاول مرة أخرى":
      "Check your connection, then try again",
    "إعادة المحاولة": "Try again",
    تخطي: "Skip",
    "الموارد الخاصة بك": "Your Resources",
    "شجرة سنابل الإحسان": "Tree of Good Deeds",
    المتجر: "Store",
    "تأكيد الشراء": "Confirm Purchase",
    "جاري الشراء...": "Purchasing...",
    الاجمالي: "Total",
    شراء: "Buy",
    إلغاء: "Cancel",
    "تمت عملية الشراء بنجاح": "Purchase completed successfully",
    إغلاق: "Close",
    تعديل: "Edit",
    "كبر الشجرة": "Grow the Tree",
    "مبروك، شجرتك تزدهر وتثمر بنجاح":
      "Congratulations, your tree is flourishing and bearing fruit successfully",

    "ثمار إحسانك تنمو لتضيء طريق الخير":
      "The fruits of your kindness grow to light the path of goodness",
    رائع: "Awesome",
    مرحبا: "Hello",
    "8 أحرف كحد أدنى": "8 characters minimum",
    رقم: "Number",
    رمز: "Code",
    "إعادة تعيين كلمة السر": "Reset Password",
    "تأمين حسابك بكلمة مرور جديدة": "Secure your account with a new password",
    "أدخل كلمة السر": "Enter Password",
    "كلمة السر": "Password",
    حفظ: "Save",
    "ليس لديك حساب؟": "Don't have an account?",
    "إنشاء حساب": "Create Account",
    "تم تغيير كلمة السر بنجاح": "Password changed successfully",
    "قم بتسجيل الدخول وابدأ في جمع الحسنات":
      "Log in and start collecting good deeds",
    "تسجيل الدخول": "Login",
    "التحقق من البريد الإلكتروني": "Email Verification",
    "هل نسيت كلمة السر؟": "Forgot Password?",
    "أدخل بريدك الإلكتروني لإعادة تعيين كلمة السر":
      "Enter your email to reset password",
    "لقد أرسلنا للتو الرمز المكون من 5 أرقام إلى":
      "We just sent a 4-digit code to",
    "أدخله أدناه:": "Enter it below:",
    "البريد الإلكتروني": "Email",
    "تأكيد الرمز": "Confirm Code",
    "أرسل الرمز": "Send Code",
    "لم تتلق رمز": "Didn't receive code",
    "بعد؟": "yet?",
    "إعادة الإرسال": "Resend",

    "اختر نوع التسجيل": "Choose Registration Type",
    طالب: "Student",
    "ولي أمر أو معلم": "Parent or Teacher",
    "مرحباً بك في": "Welcome to",
    "👋سنابل الإحسان": "👋Sanabel Al Ehsan",

    "سنابل الإحسان هو تطبيق تفاعلي يعزز القيم النبيلة عند الأطفال":
      "Sanabel Al-Ehsan is an interactive app that nurtures noble values in children",

    "ازرع سنبلة تُثمر": "Plant a grain",
    "خيراً وأجراً": "Reap goodness and reward",

    "اغتنم الفرص لفعل الخير، فالحسنات تتضاعف مع كل عمل صالح":
      "Seize the opportunity to do good—your righteous deeds multiply with every sincere act",

    "تسابقوا إلى": "Compete in doing good",
    "الخيرات، فإنها تدوم": "opportunities don’t last forever",

    "لا تفوّت الفرصة لفعل الخير، فالأجر يعمّ والحسنات تُكتب":
      "Don’t miss a chance to spread kindness— remember that rewards are multiplied, and good deeds are recorded",

    "رحلة الإحسان": "Ehsan",

    "سجل الآن واستمتع بتجربة تفاعلية تبني العطاء والانتماء":
      "Register now and begin your journey of Ehsan (Excellence in goodness)",
    "سجل الآن وابدأ": "Register now and start",

    fillEmailField: "Fill email field",
    invalidEmailFormat: "Invalid email format",
    otpSentSuccess: "OTP sent successfully",
    invalidOTP: "Invalid OTP",
    enter4DigitOTP: "Enter 4-digit OTP",
    otpVerifySuccess: "OTP verified successfully",
    "Error verifying OTP:": "Error verifying OTP:",
    "إنشاء حساب جديد": "Create New Account",
    "إنشاء حساب واستمتع بتجربة تفاعلية تبني العطاء والانتماء":
      "Create an account and enjoy an interactive experience that builds giving and belonging",
    "لقد أرسلنا للتو الرمز المكون من 4 أرقام إلى":
      "We just sent a 4-digit code to",
    "Create Password": "Create Password",
    "Secure your account with a strong password":
      "Secure your account with a strong password",
    الرجوع: "Back",
    Teacher: "Teacher",
    Parent: "Parent",
    "اسمك الأخير": "Your Last Name",
    اسمك: "Your Name",
    "اسمك الأول": "Your First Name",
    enterFirstNameAndParentName: "Enter your first name and parent's name",
    noNumbersOrSymbols: "No numbers or symbols",
    "أدخل اسمك واسم والدك": "Enter your name and your father's name",
    "أكمل حسابك واستمتع بتجربة تفاعلية تبني العطاء والانتماء":
      "Complete your account and enjoy an interactive experience that builds giving and belonging",
    "اختر النوع (بنت - ولد)": "Choose type (Girl - Boy)",
    ولد: "Boy",
    بنت: "Girl",
    numbers_only: "numbers only",
    invalid_date: "invalid date",
    "أدخل تاريخ عيد ميلادك 🎂": "Enter your birthday date 🎂",
    السنة: "Year",
    الشهر: "Month",

    متابعة: "Continue",
    "الوضع الليلي قادم قريباً": "Dark mode coming soon",
    "نحن نعمل بجد لتوفير تجربة مظلمة مريحة للعينين ترقبوا":
      "We are working hard to provide a comfortable dark experience for the eyes Stay tuned",
    "حسناً، سأنتظر": "Okay, I'll wait",

    "️اختر عددًا من التحديات اليومية والأسبوعية بحسب وقتك وظروفك":
      "Choose a number of daily and weekly challenges according to your time and circumstances",
    "اجعل الهدف الرئيسي هو الإحسان في كل عمل تقوم به لتترك أثرًا إيجابيًا في نفسك ومحيطك":
      "Make the main goal practicing excellence in every action you take to leave a positive impact on yourself and your surroundings",
    "الحرص على التوازن بين العلاقات وعدم إهمال أي جانب":
      "Strive for balance in relationships and don't neglect any aspect",
    "التدرج في الأهداف وتجنب إرهاق النفس في بداية التحدي":
      "Progress gradually in goals and avoid overwhelming yourself at the beginning of the challenge",

    "توصيات عامة": "General Recommendations",
    ابدأ: "Start",
    "سنابل الإحسان": "Sanabel Al Ehsan",
    "اختر سنبلة من سنابل الاحسان": "Choose a Sanabel",

    القاهرة: "Cairo",
    الإسكندرية: "Alexandria",
    تحديات: "Challenges",
    تمت: "Completed",
    "هل تريد حذف الحساب؟": "Do you want to delete the account?",
    "تأكد من عدم إمكانية التراجع عن هذا الإجراء":
      "Make sure this action cannot be undone",
    حذف: "Delete",
    "وقت غير صالح": "Invalid time",
    "منذ لحظات": "Moments ago",
    "منذ {{count}} دقيقة": "{{count}} minute ago",
    "منذ {{count}} ساعة": "{{count}} hour ago",
    "منذ {{count}} يوم": "{{count}} day ago",
    "منذ {{count}} أسبوع": "{{count}} week ago",
    "منذ {{count}} شهر": "{{count}} month ago",
    "وقت غير معروف": "Unknown time",
    "تاريخ غير صالح": "Invalid date",
    الكل: "All",
    اليوم: "Today",
    الأسبوع: "Week",

    "جاري تحميل النشاطات": "Loading activities",
    "لا توجد نشاطات": "No activities",
    "لم تقم بإكمال أي مهام بعد": "You haven't completed any tasks yet",
    "لا توجد نشاطات في هذه الفترة": "No activities in this period",
    سنبلة: "Sanabel",
    "مجموع النشاطات": "Total Activities",
    "كل الوقت": "All Time",
    "هذا الأسبوع": "This Week",
    "هذا الشهر": "This Month",
    الشخصية: "Character",
    الشعر: "Hair",
    القميص: "Shirt",
    الخلفية: "Background",
    البشرة: "Skin",
    "نمط الخلفية": "Background Pattern",
    "لون الخلفية": "Background Color",
    "لون الشعر": "Hair Color",
    "لون البشرة": "Skin Color",
    "اختر شخصيتك": "Choose your character",
    "صمم الشخصية المثالية للمغأمرة":
      "Design the perfect character for adventure",
    "العلاقة مع الله": "Relationship with God",
    "العلاقة مع النفس": "Relationship with Self",
    "العلاقة مع الأسرة والمجتمع": "Relationship with Family and Society",
    "العلاقة مع الأرض والكون": "Relationship with Earth and Universe",
    "مجموع التحديات": "Total Challenges",
    "كلمة السر القديمة غير صحيحة": "Old password is incorrect",
    "كلمة السر الجديدة لا تلبي المتطلبات":
      "New password doesn't meet requirements",
    "كلمة السر القديمة": "Old Password",
    "هل لديك حساب؟": "Do you have an account?",
    المرحلة: "Stage",
    "No trophies found Complete challenges to earn trophies!":
      "No trophies found Complete challenges to earn trophies!",
    "تغيير إلى العربية": "Change to Arabic",
    "تغيير إلى الإنجليزية": "Change to English",
    "تغيير كلمة المرور": "Change Password",
    "سياسة الخصوصية": "Privacy Policy",
    "مركز المساعدة": "Help Center",
    "دليل الصفحات": "Page Guides",
    "تفعيل الوضع الداكن": "Enable Dark Mode",
    "المؤثرات الصوتية": "Sound effects",
    "تسجيل الخروج": "Logout",
    "مساعدة هذه الصفحة": "Page Help",
    "المستوى التالي": "Next Level",
    المستوى: "Level",
    "مستوى الشارة": "Badge Level",
    "تم إنجاز ": "Completed ",
    نقطة: "Points",
    "تم إنجاز": "Completed",
    الميداليات: "Medals",
    "إجمالي السنابل": "Total Sanabel",
    "جوائز أخرى": "Other Rewards",
    جائزة: "Trophy",
    "هيا بنا نصنع الخير معًا": "Let's create goodness together",
    "نقطة خبرة للوصول إلى المستوى التالي":
      "experience points to reach the next level",
    "التحديات التي تم إنجازها اليوم": "Challenges completed today",
    "جاري التحميل": "Loading",
    "لوحة المتصدرين": "Leaderboard",
    الأول: "First",
    الثاني: "Second",
    الثالث: "Third",
    الرابع: "Fourth",
    الخامس: "Fifth",
    السادس: "Sixth",
    السابع: "Seventh",
    الثامن: "Eighth",
    التاسع: "Ninth",
    العاشر: "Tenth",
    تصفية: "Filter",
    "لا توجد نتائج تطابق المرشحات المحددة":
      "No results match the specified filters",
    "لا توجد بيانات كافية لعرض لوحة المتصدرين":
      "Insufficient data to display leaderboard",
    "مسح الكل": "Clear All",
    "التحديات تعزز قيم الإحسان والتعاون لدى الأطفال":
      "Challenges enhance values of kindness and cooperation in children",
    المركز: "Rank",
    "نظرة عامة": "Overview",
    الشجرة: "Tree",
    الجوائز: "Trophies",
    النشاط: "Activity",
    "Failed to copy text:": "Failed to copy text:",
    "الملف الشخصي": "Profile",
    "تم النسخ بنجاح": "Copied successfully",
    "نقاط خبرة عالية": "High experience points",
    "نقاط خبرة متوسطة": "Medium experience points",
    "نقاط خبرة منخفضة": "Low experience points",
    "الاسم (ي-أ)": "Name (Z-A)",
    "نقاط الخبرة (عالي-منخفض)": "Experience points (High-Low)",
    "نقاط الخبرة (منخفض-عالي)": "Experience points (Low-High)",
    "عدد الطلاب (قليل-كثير)": "Number of students (Few-Many)",
    "مستوى عالي (10+)": "High level (10+)",
    "مستوى متوسط (5-9)": "Medium level (5-9)",
    "مستوى منخفض (1-4)": "Low level (1-4)",
    "الاسم (أ-ي)": "Name (A-Z)",
    "المستوى (منخفض-عالي)": "Level (Low-High)",
    "تصفية وترتيب": "Filter and Sort",
    "تصفية حسب": "Filter by",
    "ترتيب حسب": "Sort by",
    تطبيق: "Apply",
    "مهمة مكررة!": "Duplicate task!",
    "بعض الطلاب أكملوا هذه المهمة بالفعل اليوم":
      "Some students have already completed this task today",
    "تأكيد تسجيل المهمة": "Confirm Task Registration",
    "الطلاب المختارين": "Selected Students",
    المحددة: "Selected",
    لعدد: "for number",
    حسناً: "Okay",
    "Some students have already completed this task today":
      "Some students have already completed this task today",
    "Error adding progress:": "Error adding progress:",
    الفصول: "Classes",
    الطلاب: "Students",
    "اختر الفئة": "Choose Category",
    "اختر النوع": "Choose Type",
    "اختر المهمة": "Choose Task",
    "ابحث عن فصل": "Search for class",
    "ابحث عن طالب": "Search for student",
    "لا يوجد فصول": "No classes",
    "لا يوجد طلاب": "No students",

    "تسجيل الابناء": "Register Children",
    "دعوة الابناء": "Invite Children",
    "عرض الابناء": "View Children",
    "ابدأ بدعوة طفلك": "Start by inviting your child",
    "جاري التحقق من الأبناء المرتبطين...": "Checking linked children...",
    "للوصول إلى التسجيل والمتابعة والتحديات، اربط حساب طفلك أولاً باستخدام كود الدعوة الخاص به.":
      "To access tracking, registration, and challenges, first link your child's account using their invitation code.",
    "كيف يحصل الطفل على الكود؟": "How does your child get the code?",
    "يدخل الطفل إلى حسابه ثم يفتح الملف الشخصي وينسخ كود الربط الظاهر في معلومات الحساب.":
      "Your child logs into their account, opens the profile page, and copies the link code shown in account information.",
    "بعد الربط ستظهر لك أدوات المتابعة والتسجيل هنا.":
      "After linking, the tracking and registration tools will appear here.",
    "دعوة طفل الآن": "Invite a Child Now",

    "دعوة الطلاب": "Invite Your Children",
    "تسجيل الطلاب": "Register Students",
    "سجل إنجازات الطلاب الفردية وتقدمهم":
      "Record individual student achievements and their progress",

    "أدخل كود دعوة الطفل": "Enter child invitation code",
    "أرسل دعوات للطلاب للانضمام إلى فصولك":
      "Send invitations to your children to join you",
    "أرسل دعوات للأبناء للانضمام إلى فصولك":
      "Send invitations to children to join your classes",
    "عرض الطلاب": "View Students",
    "تصفح وأدر طلابك وفصولك": "Browse and manage your students and classes",
    "عرض التحديات": "View Challenges",
    "استكشف التحديات والأنشطة المتاحة":
      "Explore available challenges and activities",
    "إدارة إعدادات حسابك": "Manage your account settings",
    "كيفية العثور على كود الطفل": "How to find your child's code",
    "يمكن للطالب العثور على الكود الخاص به من خلال الدخول إلى حسابه والذهاب إلى صفحة الملف الشخصي سيجد الكود معروضاً بوضوح في قسم معلومات الحساب":
      "The kids can find their code by logging into their account and going to the profile page They will find the code clearly displayed in the account information section",
    "يمكن للطفل العثور على الكود الخاص به من خلال الدخول إلى حسابه والذهاب إلى صفحة الملف الشخصي. سيجد الكود معروضاً بوضوح في قسم معلومات الحساب":
      "Your child can find their code by logging into their account and going to the profile page. The code is shown clearly in account information.",
    "أدخل كود دعوة الطالب": "Enter student invitation code",
    بحث: "Search",
    "تهانينا!": "Congratulations",
    "لقد تمت دعوة الطالب بنجاح": "Student has been successfully invited",
    "الذهاب إلى الصفحة الرئيسية": "Go to Home Page",
    "المستوى (الأعلى)": "Level (Highest)",
    "المستوى (الأدنى)": "Level (Lowest)",
    فلتر: "Filter",
    "تسجيل الفصول": "Register Classes",
    "سجل إنجازات وتابع تقدم الطلاب في فصلك الدراسي":
      "Record achievements and track student progress in your classroom",
    "عرض الطلاب والفصول": "View Students and Classes",
    "مرحباً أستاذ": "Hello Teacher",

    "إدارة فصولك وطلابك بسهولة": "Manage your classes and students easily",
    "النقاط (الأعلى)": "Points (Highest)",
    "النقاط (الأدنى)": "Points (Lowest)",
    "عدد الطلاب (الأعلى)": "Number of students (Highest)",
    "عدد الطلاب (الأدنى)": "Number of students (Lowest)",
    "استعرض الطلاب": "Browse Students",

    //

    "تم العثور على": "Found",

    "لا توجد نتائج للبحث": "No search results found",
    "لا توجد نتائج تطابق بحثك": "No results match your search",
    "مسح البحث": "Clear Search",
    ذكور: "Males",
    إناث: "Females",

    "تصفية النتائج": "Filter Results",
    "المرحلة الدراسية": "Academic Stage",
    "جميع المراحل": "All Stages",
    "الفصل الدراسي": "Class",
    "جميع الفصول": "All Classes",
    "جارٍ تحميل الفصول": "Loading classes",
    النوع: "Gender",
    "جميع الطلاب": "All Students",
    "تطبيق التصفية": "Apply Filter",
    "إعادة تعيين": "Reset",

    فصل: "Class",
    طلاب: "Students",
    فصول: "Classes",

    //

    // Sanabel Al-Ihsan - Arabic to English Translation
    // Main Title
    "سنابل الإحسان في العلاقة مع الله":
      "Grains of Excellence in Relationship with Allah",
    // Section 1: Prayer
    الصلاة: "Prayer",

    "احرص على أداء صلاة الفجر في وقتها":
      "Be keen to perform Fajr prayer on time",
    "احرص على أداء صلاة الظهر في وقتها":
      "Be keen to perform Dhuhr prayer on time",
    "احرص على أداء صلاة العصر في وقتها":
      "Be keen to perform Asr prayer on time",
    "احرص على أداء صلاة المغرب في وقتها":
      "Be keen to perform Maghrib prayer on time",
    "احرص على أداء صلاة العشاء في وقتها":
      "Be keen to perform Isha prayer on time",
    "احرص على الذهاب إلى المسجد للصلاة":
      "Strive to go to the mosque for prayer",
    "صلِ ركعتين قيام ليل": "Pray two rak'ahs of night prayer",

    // Section 2: Fasting
    الصيام: "Fasting",

    "صم يومًا من الأيام المستحب صيامها":
      "Fast a day from the recommended days for fasting",
    "شارك في تحضير إفطار للصائمين":
      "Participate in preparing iftar for the fasting",
    "تجنب الغيبة والكلام السيئ أثناء صيامك":
      "Avoid backbiting and bad speech during your fast",
    "صم عن استخدام الهاتف أو مواقع التواصل لفترة معينة":
      "Fast from using the phone or social media for a certain period",
    "اقضِ يومًا بالتقليل من الطعام والتركيز على العبادة":
      "Spend a day reducing food and focusing on worship",

    // Section 3: Charity
    الصدقة: "Sadaqa (Charity and Good deeds)",

    "تبرع بجزء من مصروفك اليومي للمحتاجين":
      "Donate part of your daily allowance to those in need",
    "ساهم في شراء طعام أو ملابس لشخص محتاج":
      "Contribute to buying food or clothes for someone in need",
    "ضع صدقة في صندوق تبرعات أو جمعية خيرية":
      "Put charity in a donation box or charitable organization",
    "قدم الماء أو الطعام للحيوانات التي تراها":
      "Provide water or food to animals you see",
    "شارك في مشروع خيري لتحسين حياة الآخرين":
      "Participate in a charitable project to improve others' lives",

    // Section 4: Forgiveness and Pardoning
    "العفو والصفح": "Forgiveness and Pardoning",

    "سامح شخصًا أساء إليك دون انتظار اعتذار":
      "Forgive someone who wronged you without waiting for an apology",
    "تذكر موقفًا سامح فيه النبي ﷺ وطبقه في حياتك":
      "Remember a situation where the Prophet ﷺ forgave and apply it in your life",
    "ادعُ بالخير لشخص أخطأ في حقك": "Pray for good for someone who wronged you",
    "اجلس مع صديق بينك وبينه خلاف وحاول إنهاء النزاع":
      "Sit with a friend you have a dispute with and try to end the conflict",
    "ابدأ اليوم بنية الصفح عن أي خطأ قد يحدث":
      "Start the day with the intention to forgive any mistake that may occur",

    // Section 5: Gratitude
    الشكر: "Gratitude",

    "اكتب قائمة بثلاث نعم أنعم الله بها عليك اليوم":
      "Write a list of three blessings Allah bestowed upon you today",
    "اشكر شخصًا ساعدك بكلمة أو فعل":
      "Thank someone who helped you with a word or deed",
    'ردد يوميًا: "اللهم لك الحمد حتى ترضى"':
      "Repeat daily: 'O Allah, praise be to You until You are pleased",
    "شارك في نشر الوعي عن قيمة الشكر بين أصدقائك":
      "Participate in spreading awareness about the value of gratitude among your friends",
    "توقف للحظات يوميًا لتفكر في نعمة واحدة لا تدرك أهميتها في حياتك":
      "Stop for moments daily to think about one blessing whose importance you don't realize in your life",

    // Section 6: Patience
    الصبر: "Patience",

    "اقرأ قصة عن صبر الأنبياء وتعلم منها":
      "Read a story about the patience of the prophets and learn from it",
    "واجه موقفًا صعبًا بالصمت والتفكير قبل الرد":
      "Face a difficult situation with silence and thinking before responding",
    "جدد نيتك بالصبر عند أداء واجب أو عمل شاق":
      "Renew your intention of patience when performing a duty or hard work",
    "ذكر نفسك يوميًا بأن الصبر مفتاح الفرج":
      "Remind yourself daily that patience is the key to relief",

    // Section 7: Remembrance
    الذكر: "Dhikr (Remembrance of Allah)",

    "اقرأ ورد يومي من القرآن الكريم":
      "Read a daily portion from the Holy Quran",
    "تدبر 10 آيات من القرآن الكريم":
      "Contemplate 10 verses from the Holy Quran",
    "اقرأ أذكار الصباح والمساء بتدبر":
      "Read morning and evening remembrances with contemplation",
    "تفكر في أحد أسماء الله وصفاته واربطها بحياتك اليومية":
      "Think about one of Allah's names and attributes and connect it to your daily life",
    "خصص وقتًا لتسبيح الله 100 مرة": "Dedicate time to glorify Allah 100 times",
    "خصص وقتًا لحمد الله 100 مرة": "Dedicate time to praise Allah 100 times",

    // Section 8: Supplication
    الدعاء: "Dua (Supplication)",

    "خصص وقتًا يوميًا للدعاء وطلب العون من الله":
      "Dedicate time daily for supplication and asking for help from Allah",
    "ادعُ الله بإلحاح لتحقيق أمنية تتمنى حدوثها":
      "Pray to Allah persistently to achieve a wish you hope to happen",
    "جهز قائمة بأسماء أشخاص تحبهم وادعُ لهم في صلاتك":
      "Prepare a list of names of people you love and pray for them in your prayer",
    "اختر دعاءً من السنة النبوية واحرص على تكراره يوميًا":
      "Choose a supplication from the Prophetic Sunnah and be keen to repeat it daily",

    // Main Title
    "سنابل الإحسان في العلاقة مع النفس":
      "Grains of Excellence in Relationship with the Self",
    // Section 9: Kindness to the Body
    "الإحسان للجسد": "Body Care",

    "مارس الرياضة يومياً لمدة 15 دقيقة": "Exercise daily for 15 minutes",
    "تناول وجبة متوازنة تشمل الخضروات والفواكه":
      "Eat a balanced meal including vegetables and fruits",
    "اشرب 6-8 أكواب من الماء": "Drink 6-8 cups of water",
    "احرص على النوم مبكرًا للحصول على 8 ساعات من الراحة":
      "Be keen to sleep early to get 8 hours of rest",
    "ابتعد عن الأطعمة غير الصحية": "Stay away from unhealthy foods",

    // Section 10: Kindness to the Mind
    "الإحسان للعقل": "Mind Nourishment",

    "اقرأ قصة أو كتاب يزيد من معرفتك":
      "Read a story or book that increases your knowledge",
    "حل لغز أو مسألة رياضية لتنشيط التفكير":
      "Solve a puzzle or math problem to activate thinking",
    "تعلم كلمة أو مهارة جديدة": "Learn a new word or skill",
    "خصص 20 دقيقة يومياً لمراجعة دروس المدرسة":
      "Dedicate 20 minutes daily to review school lessons",
    "اكتب ثلاث جمل تعبر عن فكرة جديدة تعلمتها":
      "Write three sentences expressing a new idea you learned",

    // Section 11: Kindness to the Soul
    "الإحسان للروح": "Soul Nurturing",

    "اقرأ أذكار الصباح والمساء كل يوم":
      "Read morning and evening remembrances every day",
    "اقرأ بتدبر صفحة من القرآن مع فهم معانيها":
      "Read a page from the Quran with contemplation and understanding its meanings",
    "تأمل في السماء لمدة خمس دقائق": "Contemplate the sky for five minutes",
    "تأمل في أحد المخلوقات أو النباتات لمدة خمس دقائق":
      "Contemplate one of the creatures or plants for five minutes",
    "قم بالدعاء لوالديك": "Pray for your parents",
    "اقض وقتًا في ذكر الله": "Spend time in remembrance of Allah",
    "حافظ على الصلاة في وقتها": "Maintain prayer on time",
    "قم بالدعاء لنفسك": "Pray for yourself",
    "اشكر الله على نعمة تشعر بها اليوم":
      "Thank Allah for a blessing you feel today",
    "ادعُ بالخير لأناس لا تعرفهم": "Pray for good for people you don't know",
    "امشِ في الطبيعة ولو لمسافة قصيرة":
      "Walk in nature even for a short distance",

    // Section 12: Kindness to the Heart
    "الإحسان للقلب": "Heart Refinement",

    "ابدأ صباحك بتعديد نوايا إيجابية":
      "Start your morning by listing positive intentions",
    "سامح من أخطأ في حقك دون انتظار اعتذار":
      "Forgive those who wronged you without waiting for an apology",
    "اكتب ٧ أشياء أنت ممتن لها": "Write 7 things you are grateful for",
    "اكتب 3 صفات جميلة في أحد أصدقائك أو أفراد عائلتك":
      "Write 3 beautiful qualities in one of your friends or family members",
    "إذا شعرت بالغضب، قم بعدّ حتى الرقم 10 ثم تنفس بعمق":
      "If you feel angry, count to 10 then breathe deeply",
    "مارس هواية تحبها": "Practice a hobby you love",
    "ساعد في عمل خير، مثل مساعدة زميل في الدراسة أو عمل تطوعي":
      "Help in a good deed, like helping a classmate study or volunteering",
    "اذكر مميزات شخص تعرفه بدلاً من التركيز على عيوبه":
      "Mention the advantages of someone you know instead of focusing on their flaws",
    "اكتب أو ارسم ما تشعر به": "Write or draw what you feel",
    "اشكر نفسك على السعي للإحسان": "Thank yourself for striving for excellence",
    //CHECK ERROR
    "احفظ حديث النبي ﷺ: “ليس الشديد بالصرعة، إنما الشديد الذي يملك نفسه عند الغضب”":
      "Memorize the Hadith: “The strong one is not the one who overcomes others in wrestling, but the one who controls himself in anger”",
    "قم بعمل بسيط دون إخبار أحد مثل تنظيف مكان عام أو مساعدة محتاج":
      "Do a simple deed without telling anyone, like cleaning a public place or helping someone in need",
    "خصص وقتًا لكتابة 3 نعم جديدة تشكر الله عليها في دفتر النعم":
      "Dedicate time to write 3 new blessings to thank Allah for in a gratitude journal",
    "قدم هدية بسيطة لشخص حقق نجاحًا":
      "Offer a simple gift to someone who achieved success",
    "لا تقارن نفسك بالآخرين وركز على تحسين ذاتك بدلًا من الالتفات لما لديهم":
      "Don't compare yourself to others and focus on improving yourself instead of looking at what they have",
    "ابتسم دائمًا في وجه من يقابلك":
      "Always smile in the face of those you meet",
    "اكتب قائمة بأكثر ما يثير غضبك ثم خطط لكيفية التعامل مع كل موقف":
      "Write a list of what angers you most then plan how to deal with each situation",

    // Main Title
    "سنابل الإحسان في العلاقة مع الأسرة والمجتمع":
      "Grains of Excellence in Relationship with Family and Society",
    // Section 13: Honoring Parents
    "بر الوالدين": "Honoring Parents",

    "قم بتحضير وجبة الإفطار أو العشاء للعائلة":
      "Prepare breakfast or dinner for the family",
    "رتب غرفتك أو قم بتنظيف المنزل لتخفيف الأعباء عن والديك":
      "Tidy your room or clean the house to lighten the burden on your parents",
    "اشترِ هدية رمزية وقدمها لوالدك أو والدتك":
      "Buy a symbolic gift and present it to your father or mother",
    "اكتب بطاقة شكر معبرة وأهديها لوالديك":
      "Write an expressive thank you card and give it to your parents",

    // Section 14: Maintaining Family Ties
    "صلة الرحم": "Maintaining Family Ties",

    "اتصل بأحد أقاربك الذين لم تتحدث معهم منذ فترة طويلة":
      "Call one of your relatives you haven't spoken to for a long time",
    "قم بزيارة أحد أفراد العائلة واهدِ لهم شيئًا بسيطًا":
      "Visit a family member and give them something simple",
    "شارك في ترتيب تجمع عائلي صغير":
      "Participate in organizing a small family gathering",
    "ساعد قريبًا أصغر منك في دراسته أو هواياته":
      "Help a younger relative with their studies or hobbies",

    // Section 15: Honesty and Trustworthiness
    "الصدق والأمانة": "Honesty and Trustworthiness",

    "التزم بإنجاز وعد قطعته على نفسك":
      "Commit to fulfilling a promise you made to yourself",
    "احرص على أن تقول الصدق دائمًا، حتى في المواقف الصعبة":
      "Be keen to always tell the truth, even in difficult situations",
    "قم بإعادة أي شيء اقترضته من الآخرين":
      "Return anything you borrowed from others",
    "كن أمينًا في أداء واجباتك الدراسية أو العملية":
      "Be trustworthy in performing your academic or work duties",

    // Section 16: Honoring Guests
    "إكرام الضيف": "Honoring Guests",

    "حضّر مشروبًا أو وجبة خفيفة لضيوف عائلتك":
      "Prepare a drink or light meal for your family's guests",
    "قدم كرسيك لشخص يحتاجه أكثر أثناء تجمع":
      "Offer your chair to someone who needs it more during a gathering",
    "ارحب بضيوفك بابتسامة وكلمات لطيفة":
      "Welcome your guests with a smile and kind words",

    // Section 17: Kindness to Neighbors
    "الإحسان للجار": "Kindness to Neighbors",

    "قدم لجارك هدية بسيطة مثل طبق طعام":
      "Give your neighbor a simple gift like a dish of food",
    "اسأل جارك إن كان بحاجة لأي مساعدة":
      "Ask your neighbor if they need any help",
    "شارك جارك في تنظيف أو تنظيم المنطقة المشتركة":
      "Share with your neighbor in cleaning or organizing the common area",

    // Section 18: Respecting Elders and Having Mercy on the Young
    "توقير الكبير ورحمة الصغير":
      "Respecting Elders and Having Mercy on the Young",

    "ساعد جدك أو جدتك في ترتيب المنزل أو القيام بالأعمال اليومية":
      "Help your grandfather or grandmother organize the house or do daily tasks",
    "قدم المساعدة لكبار السن في التسوق أو حمل الأغراض":
      "Offer help to elderly people in shopping or carrying items",
    "خصص وقتًا للعب مع الأطفال الصغار في عائلتك":
      "Dedicate time to play with small children in your family",

    // Section 19: Gift Exchange
    التهادي: "Gift Exchange",

    "جهز هدية بسيطة لأحد أصدقائك":
      "Prepare a simple gift for one of your friends",
    "قدم وردة لشخص تحب": "Give a flower to someone you love",
    "اكتب رسالة لطيفة لشخص تعتقد أنه بحاجة لدعمك":
      "Write a nice message to someone you think needs your support",

    // Section 20: Feeding Others
    الإطعام: "Feeding Others",

    "ساهم في توزيع وجبات غذائية على المحتاجين":
      "Contribute to distributing meals to those in need",
    "حضّر وجبة وقدمها لشخص يعمل في منطقتك":
      "Prepare a meal and offer it to someone working in your area",
    "قم بالمساهمة في مشروع خيري لتوزيع الطعام":
      "Contribute to a charitable project for food distribution",

    // Section 21: Mercy and Gentleness
    "الرحمة والرفق": "Mercy and Gentleness",

    "قم بالدعاء لشخص بينك وبينه خلاف":
      "Pray for someone you have a dispute with",
    "ادعُ بالخير للآخرين، خاصةً من تعرف أنهم يمرون بوقت صعب":
      "Pray for good for others, especially those you know are going through a difficult time",
    "كن لطيفًا مع الحيوانات وقدم لها الماء أو الطعام":
      "Be kind to animals and offer them water or food",

    // Section 22: Loyalty and Gratitude
    "الوفاء والامتنان": "Loyalty and Gratitude",

    "اكتب رسالة شكر لمعلمتك في المدرسة":
      "Write a thank you letter to your teacher at school",
    "اشكر من يساعدك يوميًا مثل سائق الحافلة أو عامل النظافة":
      "Thank those who help you daily like the bus driver or cleaner",
    "قم بعمل جميل تعبيرًا عن امتنانك لشخص دعمك":
      "Do something beautiful as an expression of gratitude to someone who supported you",

    // Section 23: Bringing Joy
    "إدخال السرور": "Bringing Joy",

    "جهز مفاجأة بسيطة لأحد أصدقائك":
      "Prepare a simple surprise for one of your friends",
    "شارك في تنظيم نشاط ترفيهي للأطفال":
      "Participate in organizing a recreational activity for children",
    "أرسل رسالة مليئة بالكلمات الإيجابية لصديق":
      "Send a message full of positive words to a friend",

    // Section 24: Comforting the Lonely and Avoiding Private Conversations
    "إيناس الوحشان وترك التناجي":
      "Comforting the Lonely and Avoiding Private Conversations",

    "اقضِ وقتًا مع شخص يشعر بالوحدة":
      "Spend time with someone who feels lonely",
    "أرسل رسالة تحفيزية لشخص تعرف أنه يمر بأيام صعبة":
      "Send a motivational message to someone you know is going through difficult days",
    "ادعُ شخصًا وحيدًا لتناول الطعام أو للمشي معك":
      "Invite a lonely person to eat or walk with you",

    // Section 25: Reconciling Between Disputing Parties
    "الإصلاح بين متخاصمين": "Reconciling Between Disputing Parties",

    "تحدث مع طرفين متخاصمين وادعُهما للحوار الهادئ":
      "Talk to two disputing parties and invite them to calm dialogue",
    "اعفو عن من أساء إليك": "Forgive those who wronged you",

    // Section 26: Smiling and Spreading Peace
    "التبسم وإفشاء السلام": "Smiling and Spreading Peace",

    "ابتسم لكل من تقابله اليوم": "Smile at everyone you meet today",
    "ألقِ السلام على أشخاص لا تعرفهم": "Greet people you don't know with peace",
    "ابدأ يومك برسالة مليئة بالسلام لأصدقائك":
      "Start your day with a message full of peace to your friends",

    // Section 27: Removing Harm from the Path
    "إماطة الأذى عن الطريق": "Removing Harm from the Path",

    "التقط القمامة من الطريق وضعها في مكانها الصحيح":
      "Pick up trash from the road and put it in its proper place",
    "ساهم في حملة تنظيف": "Contribute to a cleaning campaign",
    "أصلح شيئًا معطوبًا قد يؤذي المارة":
      "Fix something broken that might harm passersby",

    // Section 28: Cooperation
    التعاون: "Cooperation",

    "اعمل مع الآخرين بروح التعاون، فيد الله مع الجماعة":
      "Work with others in a spirit of cooperation, for Allah's hand is with the group",
    "شارك في تنظيف الصف أو المدرسة":
      "Participate in cleaning the classroom or school",
    "قم بتقديم يد العون لأفراد عائلتك في الأعمال المنزلية":
      "Offer help to your family members in household chores",

    // Section 29: Kind Words and Excellence in Speech
    "الكلمة الطيبة والإحسان في القول": "Kind Words and Excellence in Speech",

    "الكلمة الطيبة كشجرة طيبة، فازرع كلماتك كزهور في قلوب الآخرين":
      "A kind word is like a good tree, so plant your words like flowers in the hearts of others",
    "اكتب رسالة تشجيعية لشخص يحتاج إلى الدعم":
      "Write an encouraging message to someone who needs support",
    "احرص على استخدام كلمات إيجابية ولطيفة طوال اليوم":
      "Be keen to use positive and kind words throughout the day",

    // Section 30: Sharing and Selflessness
    "المشاركة والإيثار": "Sharing and Selflessness",

    "قدم الآخرين على نفسك في الخير، فالإيثار قمة الأخلاق":
      "Put others before yourself in good, for selflessness is the peak of morals",
    "شارك شخصًا محتاجًا بشيء تحبه جدًا":
      "Share something you love very much with someone in need",
    "تبرع بشيء مميز لديك لمحتاج":
      "Donate something special you have to someone in need",

    // Section 31: Fulfilling Needs and Helping Others
    "قضاء الحوائج ومساعدة الآخرين": "Fulfilling Needs and Helping Others",

    "إن السعادة في المساعدة، فلا تتأخر عن مد يد العون لمن يحتاجك":
      "Happiness lies in helping, so don't delay in extending a helping hand to those who need you",
    "شارك في إعداد وجبات وتوزيعها على المحتاجين":
      "Participate in preparing meals and distributing them to those in need",
    "ساعد صديقًا يحتاج إلى دعمك في وقت صعب":
      "Help a friend who needs your support during a difficult time",
    "نظم حملة تبرع بالملابس أو الألعاب للأطفال":
      "Organize a donation campaign for clothes or toys for children",
    "اقضِ وقتًا مع شخص وحيد أو بحاجة لدعم نفسي":
      "Spend time with someone lonely or in need of psychological support",
    "قم بزيارة للأطفال في المستشفيات لقراءة القصص أو تقديم الهدايا":
      "Visit children in hospitals to read stories or give gifts",
    "اجمع الكتب أو الألعاب أو الملابس غير المستخدمة في منزلك وتبرع بها":
      "Collect unused books, toys, or clothes in your home and donate them",
    "قدم مساعدة صغيرة لشخص لا تعرفه":
      "Offer a small help to someone you don't know",

    // Environmental Kindness
    "سنابل الإحسان في العلاقة مع الأرض والكون":
      "Grains of kindness in the relationship with earth and the universe",
    "عدم الإسراف": "Avoiding waste",
    "ابدأ حملة توعية في المدرسة أو بين أسرتك لترشيد استهلاك المياه":
      "Start an awareness campaign at school or among your family to rationalize water consumption",
    "استخدم بدائل لمنتجات البلاستيك": "Use alternatives to plastic products",
    "قم بإعادة تدوير أحد الأشياء التي لا تستخدمها":
      "Recycle one of the things you don't use",
    "حافظ على البيئة من خلال عدم إهدار الموارد (ماء، كهرباء، طعام)":
      "Preserve the environment by not wasting resources (water, electricity, food)",
    "الاحسان للمخلوقات (الطيور والحيوانات)":
      "Kindness to creatures (birds and animals)",
    "ضع وعاء ماء أو بعض الطعام للحيوانات أو الطيور خارج منزلك":
      "Place a bowl of water or some food for animals or birds outside your home",
    الغرس: "Planting",
    "ازرع نباتًا صغيرًا أو قم بتنظيف مساحة صغيرة في المدرسة أو المنطقة المحيطة بك":
      "Plant a small plant or clean a small area in the school or the area around you",
    "قم بالاعتناء بنبات موجود في المنزل":
      "Take care of a plant that exists in the house",
    "اغرس شجرة أو زهور في مكان مناسب":
      "Plant a tree or flowers in a suitable place",
    "الإحسان للأرض والنبات": "Kindness to earth and plants",
    "اقضِ 10 دقائق على الأقل في الطبيعة (حديقة، شرفة، نافذة تطل على الخارج)":
      "Spend at least 10 minutes in nature (garden, balcony, window overlooking outside)",
    "تفكر في جمال المخلوقات والنظام الكوني الذي خلقه الله":
      "Reflect on the beauty of creatures and the cosmic system that God created",
    "شارك في تنظيف شاطئ أو حديقة عامة":
      "Participate in cleaning a beach or public garden",
    "التقط القمامة من على الأرض": "Pick up trash from the ground",
    "نظم نشاط لتنظيف أو تحسين البيئة (تجميع مخلفات، ترتيب مكان)":
      "Organize an activity to clean or improve the environment (collecting waste, organizing a place)",

    // prayers
    الفجر: "Fajr",
    الشروق: "Sunrise",
    الظهر: "Dhuhr",
    العصر: "Asr",
    المغرب: "Maghrib",
    العشاء: "Isha",

    "لقد وصلت إلى الحد الأقصى المطلوب من الماء":
      "You have reached the maximum required amount of water",
    "لقد وصلت إلى الحد الأقصى المطلوب من السماد":
      "You have reached the maximum required amount of fertilizer",
    "ليس لديك سنابل كافية لإتمام الشراء":
      "You don't have enough Sanabel to complete the purchase",

    email_example: "Example: amrahmed@gmail.com",

    في: "In",

    "التحديات اليومية": "Daily Challenges",
    "خطوات بسيطة لتعلم العطاء ومساعدة الآخرين":
      "Simple steps to learn giving and helping others",
    "نظام النقاط": "Point System",
    "تتبع تقدمك والمنافسة مع الأصدقاء":
      "Track your progress and compete with friends",
    "الألعاب التعليمية": "Educational Games",
    "تعلم العطاء عبر ألعاب مسلية": "Learn giving through entertaining games",
    "تصميم تفاعلي": "Interactive Design",
    "رسومات كرتونية وأنيميشن جذابة":
      "Cartoonish graphics and engaging animations",
    "إرشادات الوالدين": "Parental Guidance",
    "خطوات عملية لتعزيز دور الأهل":
      "Practical steps to enhance the role of parents",
    "المكافآت والجوائز": "Rewards and Prizes",
    "اجمع النقاط واستبدلها بمفاجآت رائعة":
      "Collect points and exchange them for great surprises",
    "تتبع التقدم": "Progress Tracking",
    "راقب تطورك وانمِ مهاراتك بشكل مستمر":
      "Monitor your growth and develop your skills continuously",
    "لوحات المتصدرين": "Leaderboards",
    "تنافس مع أصدقائك واحتل المراكز الأولى":
      "Compete with your friends and climb to the top spots",

    "انواع السنابل": "Sanabel Types",
    "في العلاقة مع": "In relation with",
    "مع الله": "God",
    النفس: "The Self",
    "الاسرة والمجتمع": "Family and Society",
    "الارض والكون": "Earth and the Cosmos",
    "اكتشف سنابل الإحسان": "Discover Sanabel",
    "احرص على أداء الصلوات الخمس في وقتها":
      "Be keen on performing the five daily prayers on time",

    "أعظم أركان الإسلام بعد الشهادتين":
      "The greatest pillar of Islam after the two testimonies",

    "خصص وقتاً لصلاة الضحى": "Allocate time for the Duha prayer",

    "صل ركعتين قيام ليل": "Pray two Rak'ahs of night prayer (Qiyam al-Layl)",

    "جوائز السنابل": "Sanabel Rewards",
    "هذه الجوائز المميزة تكسبها عند إكمال مهام سنابل":
      "These special rewards are earned by completing Sanabel tasks",
    "هذه الجوائز تكسبها من خلال إنجازات متنوعة أخرى":
      "These rewards are earned through various other achievements",

    "اجمع نقاط الخبرة لترتفع في المستوى!":
      "Collect experience points to level up!",
    "كلما لعبت أكثر، زادت مكافآتك":
      "The more you play, the greater your rewards",
    "سنبلة زرقاء": "Blue Sanabel",
    "سنبلة حمراء": "Red Sanabel",
    "سنبلة صفراء": "Yellow Sanabel",
    "نقاط خبرة": "Experience Points",
    "مكافآت إكمال المهمة": "Task Completion Rewards",
    "عند إكمال المهام، ستحصل على المكافآت الرائعة!":
      "When you complete tasks, you will receive amazing rewards!",

    مياه: "Water",

    "قم بشراء المياه والسماد": "Buy water and fertilizer",

    "احصل على مياه لري السنابل الخاصة بك":
      "Get water to irrigate your wheat spikes",
    "عزز نمو وإنتاجية سنابلك":
      "Enhance the growth and productivity of your wheat spikes",
    اشتري: "Buy",
    "تم الشراء": "Purchased",
    أحسنت: "Well Done",
    "تم الشراء بنجاح": "Purchase completed successfully",

    "كبر شجرتك": "Grow Your Tree",
    "هذه شجرتك الآن، أحسنت": "This is your tree now, well done",
    "ازرع شجرتك": "Plant Your Tree",

    مبتدئ: "Beginner",
    مستجد: "Newbie",
    موهوب: "Talented",
    ماهر: "Skilled",
    بارع: "Skillful",
    متمرس: "Seasoned",
    متقدم: "Advanced",
    متقن: "Master",
    خبير: "Expert",

    "احصل على ميداليات": "Earn Medals",

    "نافس أصدقاءك واظهر اسمك في قائمة الأفضل":
      "Compete with your friends and show your name on the leaderboard",

    "احمد جمال": "Ahmed Gamal",
    "عمر شريف": "Omar Sherif",
    "سارة حسن": "Sara Hassan",

    "ابدأ رحلتك": "Start your Journey",
    مرحباً: "Hello",

    "تقدمك نحو الخير": "Your Progress Towards Good",
    "تابع إنجازاتك وازرع سنابل الخير كل يوم":
      "Track your achievements and plant seeds of good every day",

    "مجموع اكس بي": "Total XP",

    "تعديل الملف الشخصي": "Edit Profile",

    "Missions Finished": "Missions Finished",
    "Total Blue Points": "Total Blue Points",

    "Total Yellow Points": "Total Yellow Points",
    "Total Red Points": "Total Red Points",
    "Total Mixed Points": "Total Mixed Points",
    "Total XP": "Total XP",
    "Total Water Bought": "Total Water Bought",
    "Total Fertilizer Bought": "Total Fertilizer Bought",
    "Tree Stage": "Tree Stage",
    "Progress Tree": "Progress Tree",

    "مرحلة الشجرة": "Tree Stage",
    "المهام المكتملة": "Completed Tasks",
    "شجرة التقدم": "Progress Tree",
    "السنبلة الزرقاء": "Blue Wheat Spike",
    "السنبلة الصفراء": "Yellow Wheat Spike",
    "السنبلة الحمراء": "Red Wheat Spike",

    "الـ XP": "XP",
    الماء: "Water",

    "تهانينا! حصلت على كأس جديد": "Congrats! You earned a new trophy",
    "عدد النتائج:": "Number of results:",
    "لم تحصل على أي كؤوس حتى الآن": "You haven't earned any trophies yet",
    "جرب تغيير الفترة الزمنية للبحث": "Try changing the search time period",
    "لا يوجد إشعارات": "No notifications",
    "لا يوجد إشعارات في هذه الفترة": "No notifications during this period",
    "جاري التحميل...": "Loading...",
    "الأحدث أولاً": "Newest first",
    "الأقدم أولاً": "Oldest first",
    "ترتيب النتائج": "Sort results",

    "الفترة الزمنية": "Time period",
    الإشعارات: "Notifications",
    "تحديد الكل كمقروء": "Mark all as read",
    "تحديد كمقروء": "Mark as read",
    الأحدث: "Newest",
    الأقدم: "Oldest",
    إنجاز: "achievements",
    "لا يوجد إشعارات بعد": "No notifications yet",
    "أكمل التحديات لتحصل على كؤوس ومكافآت!":
      "Complete challenges to earn trophies and rewards!",
    "جرّب تغيير الفترة الزمنية للعثور على إشعاراتك":
      "Try changing the time range to find your notifications",
    "عرض الكل": "Show all",
    "تفعيل إشعارات الصلاة": "Enable Prayer Notifications",
    "تم تفعيل إشعارات الصلاة بنجاح!":
      "Prayer notifications enabled successfully!",
    "متصفحك لا يدعم الإشعارات.": "Your browser does not support notifications.",
    "على iPhone، أضف سنابل إلى الشاشة الرئيسية أولاً ثم افتحه من هناك لتفعيل الإشعارات.":
      "On iPhone, first add Sanabel to your Home Screen, then open it from there to enable notifications.",
    "تعذر تفعيل الإشعارات على هذا الجهاز.": "Notifications could not be enabled on this device.",
    "يجب الموافقة على الإشعارات لتفعيل هذه الخاصية.":
      "You must allow notifications to enable this feature.",
    "حدث خطأ أثناء تفعيل الإشعارات.":
      "An error occurred while enabling notifications.",
    "يجب السماح بالوصول للموقع لتحديد أوقات الصلاة بدقة.":
      "Location access is required to determine accurate prayer times.",
    "حدث خطأ أثناء الشراء. حاول مرة أخرى.":
      "Something went wrong with the purchase. Please try again.",
    "تعذر تكبير الشجرة. حاول مرة أخرى.":
      "Could not grow the tree. Please try again.",
    "أضف كمية من الماء أو السماد أولاً.": "Add some water or fertilizer first.",
    "الكمية المطلوبة غير صحيحة.": "The requested quantity is not valid.",
    "رصيد السنابل غير كافٍ.": "Your sanabel balance is not enough.",
    "لم يتم العثور على حساب الطالب.": "Student account not found.",
    "لا تملك ماءً أو سمادًا كافيًا لتكبير الشجرة.":
      "You don't have enough water or fertilizer to grow the tree.",
    "وصلت شجرتك إلى أعلى مستوى، تهانينا!":
      "Your tree has reached the highest level — congratulations!",
    "بيانات الشجرة غير متوفرة حاليًا.": "Tree data is currently unavailable.",
    "انتهت مهلة الطلب. تحقق من سرعة الإنترنت وحاول مرة أخرى.":
      "The request timed out. Check your internet speed and try again.",
    "لا يمكن الوصول إلى الخادم. تحقق من اتصالك بالإنترنت.":
      "Cannot reach the server. Check your internet connection.",
    "انتهت صلاحية جلستك. سجل الدخول مرة أخرى.":
      "Your session has expired. Please log in again.",
    "حدث خطأ في الخادم. حاول مرة أخرى بعد قليل.":
      "A server error occurred. Please try again shortly.",
    "حدث خطأ غير متوقع. حاول مرة أخرى.":
      "An unexpected error occurred. Please try again.",
    "بيانات المستخدم غير موجودة في الطلب.": "User data was not found in the request.",
    "لم يتم العثور على حساب المسؤول.": "Admin account not found.",
    "معرّف المدرسة غير صالح.": "The school ID is invalid.",
    "لم يتم العثور على المدرسة.": "School not found.",
    "الاسم مطلوب.": "Name is required.",
    "توجد مدرسة بهذا الاسم بالفعل.": "A school with this name already exists.",
    "لا يمكن حذف المدرسة لوجود بيانات مرتبطة بها. أعد تعيينها أو احذفها أولاً.":
      "This school has related records. Reassign or remove them before deleting it.",
    "معرّف الطالب مطلوب.": "Student ID is required.",
    "معرّف الطالب غير صالح.": "The student ID is invalid.",
    "المدرسة المحددة غير موجودة.": "The selected school does not exist.",
    "الفصل المحدد غير موجود.": "The selected class does not exist.",
    "الفصل المحدد لا يتبع مدرسة الطالب.":
      "The selected class does not belong to the student's school.",
    "الفصل المحدد لا يتبع المدرسة المختارة.":
      "The selected class does not belong to the selected school.",
    "المرحلة الدراسية المحددة غير موجودة.":
      "The selected grade does not exist.",
    "البريد الإلكتروني مستخدم بالفعل.": "This email address is already in use.",
    "الاسم الأول والبريد الإلكتروني ونوع الحساب مطلوبة.":
      "First name, email, and account type are required.",
    "نوع الحساب يجب أن يكون طالبًا أو معلمًا أو ولي أمر أو مسؤولًا.":
      "The account type must be Student, Teacher, Parent, or Admin.",
    "اختيار المدرسة مطلوب لهذا النوع من الحسابات.":
      "A school is required for this account type.",
    "معرّف المستخدم غير صالح.": "The user ID is invalid.",
    "لم يتم العثور على المستخدم.": "User not found.",
    "لم يتم العثور على سجل طالب لهذا المستخدم.":
      "No student record was found for this user.",
    "لم يتم العثور على سجل معلم لهذا المستخدم.":
      "No teacher record was found for this user.",
    "قائمة معرّفات الفصول غير صالحة.": "The class ID list must be an array.",
    "اسم الفصل والمرحلة الدراسية والمدرسة مطلوبة.":
      "Class name, grade, and school are required.",
    "معرّف الفصل غير صالح.": "The class ID is invalid.",
    "لم يتم العثور على الفصل.": "Class not found.",
    "لا يمكن حذف الفصل لوجود طلاب مسجلين فيه. أعد تعيينهم أو احذفهم أولاً.":
      "This class has assigned students. Reassign or remove them before deleting it.",
    "المرحلة الدراسية موجودة بالفعل في هذه المدرسة.":
      "This grade already exists in this school.",
    "معرّف المرحلة الدراسية غير صالح.": "The grade ID is invalid.",
    "لم يتم العثور على المرحلة الدراسية.": "Grade not found.",
    "يجب إدخال اسم غير فارغ.": "Name must not be empty.",
    "اسم المرحلة الدراسية موجود بالفعل في هذه المدرسة.":
      "This grade name already exists in this school.",
    "لا يمكن حذف المرحلة الدراسية لوجود طلاب أو فصول مرتبطة بها. أعد تعيينها أو احذفها أولاً.":
      "This grade has assigned students or classes. Reassign or remove them before deleting it.",
    "الفصل رقم {{number}} غير موجود.": "Class {{number}} does not exist.",
    "الفصل رقم {{number}} لا يتبع المدرسة المختارة.":
      "Class {{number}} does not belong to the selected school.",
    "قيمة {{field}} يجب أن تكون رقمًا صحيحًا موجبًا أو فارغة.":
      "{{field}} must be a positive integer or empty.",
    "لم يتم العثور على صفوف بيانات في الملف.":
      "No data rows were found in this file.",
    "تعذر قراءة هذا الملف.": "Could not read this file.",
    "لا توجد صفوف صالحة للاستيراد.": "There are no valid rows to import.",
    "اختر مدينتك": "Choose your city",
    "لم نتمكن من تحديد موقعك تلقائيًا. اختر أقرب مدينة لك.":
      "We couldn't detect your location automatically. Choose the nearest city.",
    الجيزة: "Giza",
    المنصورة: "Mansoura",
    طنطا: "Tanta",
    بورسعيد: "Port Said",
    السويس: "Suez",
    أسيوط: "Asyut",
    الأقصر: "Luxor",
    أسوان: "Aswan",
    "منذ {{days}} أيام": "{{days}} days ago",
    الرمز: "Code",
    "مجموع الجوائز": "Total Trophies",
    "يمكن للطفل العثور على الكود الخاص به من خلال الدخول إلى حسابه والذهب إلى صفحة الملف الشخصي. سيجد الكود معروضاً بوضوح في قسم معلومات الحساب":
      "The kids can find their code by logging into their account and going to the profile page. The code will be clearly displayed in the account information section.",

    رجوع: "Back",

    تسجيل: "Register",

    "تسجيل للباقي": "Register rest",

    تأكيد: "Confirm",
    "تم تسجيل المهمة بنجاح": "Task registered successfully",

    "نقاط الخبرة": "XP",

    "تحديد الكل": "Select All",
    "إلغاء تحديد الكل": "Deselect All",
    "تفاصيل الفصل": "Class Details",

    "تأكيد الإنجاز": "Achievement Confirmation",
    "هل أنت متأكد من أنك أنجزت هذه المهمة؟":
      "Are you sure you have completed this task",

    "جاري التحديث...": "Updating...",

    "🎉 مبروك! 🎉": "🎉 Congratulations! 🎉",
    "لقد أنجزت المهمة بنجاح": "You have successfully completed the task",
    "استمر في التقدم الرائع 💪": "Keep up the great progress 💪",
    "تم الإنجاز": " Mark as completed",

    مقدمة: "Introduction",
    "المعلومات التي نجمعها": "Information We Collect",
    "كيفية استخدام المعلومات": "How We Use Information",
    "مشاركة المعلومات": "Information Sharing",
    "أمان البيانات": "Data Security",
    حقوقك: "Your Rights",
    "ملفات تعريف الارتباط": "Cookies",
    "تحديثات السياسة": "Policy Updates",
    "اتصل بنا": "Contact Us",
    "نحن ملتزمون بحماية خصوصيتك. تشرح هذه السياسة كيفية جمع واستخدام وحماية معلوماتك الشخصية عند استخدام تطبيقنا.":
      "We are committed to protecting your privacy. This policy explains how we collect, use, and protect your personal information when using our application.",
    "المعلومات الشخصية مثل الاسم والعمر والبريد الإلكتروني.":
      "Personal information such as name, age, and email address.",
    "معلومات الحساب مثل اسم المستخدم وكلمة المرور.":
      "Account information such as username and password.",
    "بيانات الاستخدام مثل الأنشطة والتفاعلات داخل التطبيق.":
      "Usage data such as activities and interactions within the app.",
    "معلومات تقنية مثل نوع الجهاز وعنوان IP لتحسين الأداء.":
      "Technical information such as device type and IP address to improve performance.",
    "تقديم وتحسين خدماتنا وميزات التطبيق.":
      "Providing and improving our services and app features.",
    "تخصيص تجربة المستخدم حسب الاحتياجات.":
      "Customizing user experience according to needs.",
    "إرسال إشعارات مهمة حول الخدمة.":
      "Sending important notifications about the service.",
    "تحليل البيانات لتطوير منتجات أفضل.":
      "Analyzing data to develop better products.",
    "لا نبيع أو نؤجر معلوماتك الشخصية لأطراف ثالثة.":
      "We do not sell or rent your personal information to third parties.",
    "قد نشارك المعلومات في الحالات التالية:":
      "We may share information in the following cases:",
    "بموافقتك الصريحة والمسبقة.": "With your explicit and prior consent.",
    "للامتثال للقوانين والأنظمة المعمول بها.":
      "To comply with applicable laws and regulations.",
    "لحماية حقوقنا وسلامة المستخدمين.":
      "To protect our rights and user safety.",
    "نستخدم تقنيات التشفير المتقدمة لحماية بياناتك.":
      "We use advanced encryption technologies to protect your data.",
    "نطبق إجراءات أمنية صارمة للوصول إلى المعلومات.":
      "We implement strict security procedures for accessing information.",
    "نقوم بمراجعة وتحديث أنظمة الأمان بانتظام.":
      "We regularly review and update security systems.",
    "نحتفظ بالبيانات فقط للمدة اللازمة لتحقيق الأغراض المحددة.":
      "We retain data only for the period necessary to achieve the specified purposes.",
    "الحق في الوصول إلى بياناتك الشخصية ومراجعتها.":
      "The right to access and review your personal data.",
    "الحق في تصحيح أو تحديث معلوماتك.":
      "The right to correct or update your information.",
    "الحق في طلب حذف حسابك وبياناتك.":
      "The right to request deletion of your account and data.",
    "الحق في سحب موافقتك على معالجة البيانات.":
      "The right to withdraw your consent for data processing.",
    "الحق في تقديم شكوى إلى السلطات المختصة.":
      "The right to file a complaint with competent authorities.",
    "نستخدم ملفات تعريف الارتباط لتحسين تجربة التصفح.":
      "We use cookies to improve browsing experience.",
    "يمكنك التحكم في إعدادات ملفات تعريف الارتباط من متصفحك.":
      "You can control cookie settings from your browser.",
    "بعض الميزات قد لا تعمل بشكل صحيح بدون ملفات تعريف الارتباط.":
      "Some features may not work properly without cookies.",
    "قد نقوم بتحديث هذه السياسة من وقت لآخر.":
      "We may update this policy from time to time.",
    "سيتم إشعارك بأي تغييرات مهمة عبر التطبيق أو البريد الإلكتروني.":
      "You will be notified of any important changes via the app or email.",
    "استمرار استخدام التطبيق يعني موافقتك على السياسة المحدثة.":
      "Continued use of the app means your agreement to the updated policy.",
    "إذا كان لديك أي أسئلة حول سياسة الخصوصية، يرجى التواصل معنا.":
      "If you have any questions about the privacy policy, please contact us.",
    "يمكنك إرسال استفساراتك عبر البريد الإلكتروني أو من خلال التطبيق.":
      "You can send your inquiries via email or through the app.",
    "نحن ملتزمون بالرد على جميع الاستفسارات في أقرب وقت ممكن.":
      "We are committed to responding to all inquiries as soon as possible.",

    "رصيد غير كافي": "Insufficient Balance",
    "تحتاج إلى المزيد من السنابل لإتمام هذه العملية":
      "You need more Sanabel to complete this action",
    "السنابل المطلوبة": "Required Sanabel",
    لديك: "You have",
    تحتاج: "You need",
    إضافية: "more",
    فهمت: "Got it",
    "!اختر شخصيتك": "Choose your character",

    login_failed: "Email or password is incorrect. Please try again.",
    account_deleted_message:
      "Your account was deleted. Contact an administrator if you think this is a mistake.",

    "لم يقم الطالب بإكمال أي مهام بعد":
      "The student hasn't completed any tasks yet",

    "إضافة مهمة جديدة": "Add New Mission",

    إضافة: "Add",
    "قائمة المهام": "Task List",
    "ابحث عن مهمة": "Search for a task",

    معلقة: "Pending",
    مكتملة: "Completed",
    "لا توجد مهام": "No tasks found",
    "اضغط على زر الإضافة لإنشاء مهمة جديدة":
      "Click the add button to create a new task",

    "🌳 تهانينا! لقد أكملت شجرة الإحسان بالكامل! 🌟 أداؤك مميز، وجهودك الرائعة تُثبت أنك بطل الإحسان الحقيقي 💚 استمر في زراعة الخير يومًا بعد يوم! 🙌✨":
      "🌳 Congratulations! You’ve fully completed the Sanabel Tree! 🌟 Your performance is outstanding, and your efforts prove you're a true hero of kindness 💚 Keep planting goodness day after day! 🙌✨",

    "خطأ: المهمة لا تحتوي على معرف صحيح":
      "Error: Task does not have a valid ID",
    "الرجاء اختيار مهمة أولاً.": "Please select a task first.",

    "فشل في تحديد المهمة كمكتملة:": "Failed to mark task as completed:",
    "حدث خطأ أثناء تحديد المهمة كمكتملة.":
      "An error occurred while marking the task as completed.",
    "تعذر تحديد هذه المهمة. حاول اختيارها من جديد.":
      "Could not identify this task. Please select it again.",
    "حدث خطأ أثناء تسجيل المهمة. حاول مرة أخرى.":
      "An error occurred while recording the task. Please try again.",
    "تعذر الاتصال بالخادم. تحقق من اتصالك بالإنترنت وحاول مرة أخرى.":
      "Could not reach the server. Check your internet connection and try again.",

    مستخدم: "User",
    "سجل كفرد للاستفادة من جميع الميزات":
      "Register as an individual to benefit from all features",
    مدرسة: "School",
    "يمكن أن يكون معلمًا أو ولي أمر": "Can be a teacher or a parent",

    "لون واحد": "Single Color",
    تدرج: "Gradient",
    نقاط: "Dots",
    خطوط: "Lines",

    دعوة: "Invite",
    "عذراً!": "Sorry!",
    "لم يتم العثور على طالب بهذا الكود": "No student found with this code",
    "حدث خطأ أثناء البحث عن الطالب":
      "An error occurred while searching for the student",
    "فشلت عملية الدعوة": "Invitation failed",
    "حدث خطأ أثناء دعوة الطالب": "An error occurred while inviting the student",
    "يرجى التحقق من الكود والمحاولة مرة أخرى":
      "Please check the code and try again",
    "حاول مرة أخرى": "Try Again",

    "يرجى إدخال كود الطالب": "Please enter the student code",
    "خطأ في المصادقة": "Authentication error",
    "حدث خطأ في الاتصال بالسيرفر": "Connection error to server",
    "الطالب مرتبط بالفعل بولي أمر آخر":
      "Student is already connected to another parent",

    // Admin translations
    "لوحة تحكم المشرف": "Admin Dashboard",
    المدارس: "Schools",
    "بيانات المستخدمين": "User Data",
    "عرض كل المستخدمين (طلاب، معلمون، أولياء أمور، مشرفون) وإعادة تعيين كلمات المرور":
      "View all users (students, teachers, parents, admins) and reset passwords",
    "إنشاء وتعديل وحذف المدارس": "Create, edit, and delete schools",
    "البحث في كل الطلاب وتعديل بياناتهم":
      "Search all students and edit their data",
    "ابحث باسم المدرسة": "Search by school name",
    "لا توجد نتائج": "No results",
    "تم حذف المدرسة بنجاح": "School deleted successfully",
    "لا يمكن حذف المدرسة، تحتوي على": "Cannot delete school, it contains",
    معلم: "Teacher",
    "حدث خطأ أثناء حذف المدرسة": "An error occurred while deleting the school",
    "إضافة مدرسة": "Add School",
    "تعديل المدرسة": "Edit School",
    "اسم المدرسة": "School Name",
    "رابط الصورة (اختياري)": "Image URL (optional)",
    "تم إنشاء المدرسة بنجاح": "School created successfully",
    "تم تحديث المدرسة بنجاح": "School updated successfully",
    "اسم المدرسة مطلوب": "School name is required",
    "يوجد مدرسة بنفس هذا الاسم بالفعل":
      "A school with this name already exists",
    "حدث خطأ أثناء الحفظ": "An error occurred while saving",
    "تعذر تحميل بيانات المدرسة": "Failed to load school data",
    "جاري الحفظ...": "Saving...",
    "تعذر حفظ الصورة الشخصية، حاول مرة أخرى":
      "Couldn't save your avatar, please try again",
    "تخطي الجولة التعريفية؟": "Skip the tutorial?",
    "لن تتمكن من رؤية هذه الخطوات مرة أخرى":
      "You won't be able to see these steps again",
    "كل المدارس": "All Schools",
    "كل الفصول": "All Classes",
    "كل المراحل": "All Stages",
    primary: "Primary",
    preparatory: "Preparatory",
    secondary: "Secondary",
    "تم حذف الطالب بنجاح": "Student deleted successfully",
    "حدث خطأ أثناء حذف الطالب": "An error occurred while deleting the student",
    "تعديل بيانات الطالب": "Edit Student Data",
    "مهام مكتملة": "Completed Tasks",
    "الاسم الأول": "First Name",
    "اسم العائلة": "Last Name",
    "اختر المرحلة": "Select Stage",
    المدرسة: "School",
    "اختر المدرسة": "Select School",
    "بدون فصل": "No Class",
    "حفظ التعديلات": "Save Changes",
    "تم تحديث بيانات الطالب بنجاح": "Student data updated successfully",
    "تعذر تحميل بيانات الطالب": "Failed to load student data",
    "حذف الطالب": "Delete Student",
    "تأكيد حذف الطالب": "Confirm Student Deletion",
    الفصل: "Class",

    // Generic error-toast fallbacks (used when the server gave no message)
    otpSendFailed: "Couldn't send the code. Please try again.",
    registrationFailed: "Registration failed. Please try again.",
    networkError:
      "Couldn't reach the server. Check your connection and try again.",

    // Known server error messages (surfaced as-is in English mode)
    "Incorrect email or password": "Incorrect email or password",
    "Email already in use": "Email already in use",
    "User not found": "User not found",
    "Email is already registered. Login or use another email.":
      "Email is already registered. Login or use another email.",
    "OTP record not found. Verify OTP before registering.":
      "OTP record not found. Verify OTP before registering.",
    "OTP not verified. Verify OTP before resetting password.":
      "OTP not verified. Verify OTP before resetting password.",
    "Registration failed": "Registration failed",
    "Error sending OTP": "Error sending OTP",
    "Error verifying OTP": "Error verifying OTP",
    "A valid email and 4-digit OTP are required":
      "A valid email and 4-digit OTP are required",
    "Error resetting password": "Error resetting password",
    "Incorrect current password": "Incorrect current password",
    "An error occurred during login": "An error occurred during login",

    // ─── Admin UserData Panel ────────────────────────────────────────
    "admin.backToDashboard": "Back to Dashboard",
    "admin.userDataTitle": "User Management",
    "admin.languageToggle": "العربية",

    // Tabs
    "admin.tab.users": "All Users",
    "admin.tab.students": "Students",
    "admin.tab.teachers": "Teachers",
    "admin.tab.parents": "Parents",
    "admin.tab.admins": "Admins",
    "admin.tab.classes": "Classes",
    "admin.tab.organizations": "Schools",
    "admin.tab.grades": "Grades",
    "admin.tab.scores": "Scores & Progress",
    "admin.tab.history": "Task History",
    "admin.tab.app_version": "App Releases",
    "admin.analytics.title": "Analytics",
    "admin.analytics.overview": "Overview",
    "admin.analytics.missions": "Missions",
    "admin.analytics.people": "People",
    "admin.analytics.organizations": "Organizations",
    "admin.analytics.approvals": "Approvals",
    "admin.analytics.assignments": "Assignments",
    "admin.analytics.totalUsers": "Total Users",
    "admin.analytics.soloUsers": "Solo Users",
    "admin.analytics.schoolStudents": "School Students",
    "admin.analytics.schoolAdmins": "School Admins",
    "admin.analytics.superAdmins": "Super Admins",
    "admin.analytics.completionsToday": "Completions Today",
    "admin.analytics.completionsWeek": "Completions This Week",
    "admin.analytics.completionsMonth": "Completions This Month",
    "admin.analytics.completionsAllTime": "Completions All Time",
    "admin.analytics.pendingApprovals": "Pending Approvals",
    "admin.analytics.approved": "Approved",
    "admin.analytics.denied": "Denied",
    "admin.analytics.approvalRate": "Approval Rate",
    "admin.analytics.noResolvedYet": "No requests resolved yet",
    "admin.analytics.noneObserved": "None recorded yet",
    "admin.analytics.notMeasured": "Not measured yet",
    "admin.analytics.notMeasuredNote": "These figures have no historical source in the database. They become available once reward and economy events are recorded, rather than being estimated from current mission values.",
    "admin.analytics.completionTrend": "Completion Trend",
    "admin.analytics.topToday": "Most Completed Today",
    "admin.analytics.topAllTime": "Most Completed All Time",
    "admin.analytics.byCategory": "By Category",
    "admin.analytics.bySource": "By Completion Source",
    "admin.analytics.bySourceNote": "Source is recorded from the September 2026 migration onward; earlier completions were classified during backfill.",
    "admin.analytics.mission": "Mission",
    "admin.analytics.category": "Category",
    "admin.analytics.completions": "Completions",
    "admin.analytics.source": "Source",
    "admin.analytics.student": "Student",
    "admin.analytics.type": "Type",
    "admin.analytics.date": "Date",
    "admin.analytics.confirmedBy": "Confirmed By",
    "admin.analytics.activeStudents": "Active Students",
    "admin.analytics.avgPerActive": "Avg Completions / Active Student",
    "admin.analytics.soloCompletions": "Solo Completions",
    "admin.analytics.schoolCompletions": "School Completions",
    "admin.analytics.mostActive": "Most Active Students",
    "admin.analytics.inactive": "No Activity In Range",
    "admin.analytics.inactiveNote": "Measured by mission completions. Login events are not recorded, so this is not a session-based metric.",
    "admin.analytics.lastCompletion": "Last Completion",
    "admin.analytics.never": "Never",
    "admin.analytics.whoCompleted": "People Who Completed Missions",
    "admin.analytics.searchPlaceholder": "Search student, email or mission",
    "admin.analytics.total": "Total",
    "admin.analytics.perOrganization": "Per Organization",
    "admin.analytics.organization": "Organization",
    "admin.analytics.teacherApprovals": "Teacher Approvals",
    "admin.analytics.parentApprovals": "Parent Approvals",
    "admin.analytics.avgResolution": "Avg Resolution",
    "admin.analytics.minutes": "min",
    "admin.analytics.oldestPending": "Oldest Pending Requests",
    "admin.analytics.pendingHours": "Pending (hours)",
    "admin.analytics.noPending": "No pending requests",
    "admin.analytics.slowestOrgs": "Slowest Organizations",
    "admin.analytics.resolved": "Resolved",
    "admin.analytics.activeTodos": "Active To-Do Items",
    "admin.analytics.pendingApproval": "Pending Approval",
    "admin.analytics.completedTodos": "Completed",
    "admin.analytics.completionRate": "Completion Rate",
    "admin.analytics.bySourceType": "By Assignment Source",
    "admin.analytics.items": "Items",
    "admin.analytics.noData": "No data for this period",

    // App Version & Update Prompts
    "appUpdate.newVersionTitle": "New Update Available!",
    "appUpdate.whatsNew": "What's new in this version:",
    "appUpdate.defaultNotes": "Performance improvements and bug fixes.",
    "appUpdate.remindLater": "Later",
    "appUpdate.updateNow": "Update Now",
    "appUpdate.forceUpdateTitle": "Mandatory Update Required",
    "appUpdate.forceUpdateSubtitle": "To continue using the application, please update to the latest version available.",
    "appUpdate.requiredVersion": "Minimum Required:",
    "appUpdate.targetVersion": "Available Version:",
    "appUpdate.defaultForceNotes": "This update includes critical changes to ensure security, stability, and backend compatibility.",
    "appUpdate.updateGooglePlay": "Update on Google Play Store",
    "appUpdate.updateAppStore": "Update on Apple App Store",
    "appUpdate.maintenanceTitle": "Scheduled System Maintenance",
    "appUpdate.maintenanceMessage": "We are currently performing scheduled maintenance to improve your experience. We will be back online shortly. Thank you for your patience.",
    "appUpdate.tryAgain": "Try Again",

    // Admin App Version Control
    "admin.versionControl.title": "App Releases & Version Control",
    "admin.versionControl.subtitle": "Manage mandatory & optional updates for Android and iOS mobile apps and maintenance mode.",
    "admin.versionControl.refresh": "Refresh Data",
    "admin.versionControl.saveSuccess": "App version configurations saved successfully!",
    "admin.versionControl.saveError": "Failed to save app version configurations",
    "admin.versionControl.fetchError": "Failed to load app version configurations",
    "admin.versionControl.maintenanceMode": "Global Maintenance Mode",
    "admin.versionControl.maintenanceDesc": "When maintenance mode is enabled, mobile users will be locked out and presented with a maintenance screen.",
    "admin.versionControl.androidApp": "Android App",
    "admin.versionControl.iosApp": "iOS App",
    "admin.versionControl.forceUpdateOn": "Mandatory Update Active",
    "admin.versionControl.softUpdateOn": "Optional Update",
    "admin.versionControl.latestVersion": "Latest Version",
    "admin.versionControl.minRequiredVersion": "Minimum Required Version",
    "admin.versionControl.forceUpdateSwitch": "Force Mandatory Update",
    "admin.versionControl.forceUpdateHelp": "When enabled, users with older versions are blocked until they update.",
    "admin.versionControl.storeUrl": "Google Play Store URL",
    "admin.versionControl.storeUrlIos": "Apple App Store URL",
    "admin.versionControl.notesAr": "Release Notes (Arabic)",
    "admin.versionControl.notesEn": "Release Notes (English)",
    "admin.versionControl.guideTitle": "How do mobile updates work?",
    "admin.versionControl.softUpdate": "Optional Update (Soft):",
    "admin.versionControl.softUpdateExpl": "Users see a popup with release notes and can update or skip.",
    "admin.versionControl.hardUpdate": "Mandatory Update (Force):",
    "admin.versionControl.hardUpdateExpl": "The app is locked with a screen directing users to Google Play / App Store.",

    // Stats cards
    "admin.stats.totalUsers": "Total Users",
    "admin.stats.totalStudents": "Students",
    "admin.stats.totalTeachers": "Teachers",
    "admin.stats.totalParents": "Parents",
    "admin.stats.totalAdmins": "Admins",
    "admin.stats.playingStudents": "Active Players",
    "admin.stats.totalXp": "Total XP",
    "admin.stats.totalSanabel": "Total Sanabel",
    "admin.stats.topLevel": "Highest Level",
    "admin.stats.completedTasks": "Completed Tasks",
    "admin.stats.completedToday": "Completed Today",

    // Table headers
    "admin.th.id": "ID",
    "admin.th.name": "Name",
    "admin.th.email": "Email",
    "admin.th.role": "Role",
    "admin.th.verified": "Verified",
    "admin.th.createdAt": "Created At",
    "admin.th.grade": "Stage",
    "admin.th.organization": "School",
    "admin.th.class": "Class",
    "admin.th.xp": "XP",
    "admin.th.childCount": "Linked Children",
    "admin.th.className": "Class Name",
    "admin.th.studentCount": "Students",
    "admin.th.orgName": "School Name",
    "admin.th.gradeName": "Grade Name",
    "admin.th.actions": "Actions",
    "مضافة بالفعل في قائمتك": "Already in your list",
    "تمت الإضافة للقائمة": "Added to list",
    "هذه المهمة مضافة بالفعل إلى قائمتك اليومية": "This mission is already in your daily list",
    "جميع مهام هذا القسم مضافة بالفعل إلى قائمتك اليومية": "All missions in this section are already in your daily list 🎉",
    "اختر مهمة لإضافتها إلى قائمتك": "Choose a mission to add to your list",
    "متاح للإضافة": "Available to add",
    "تمت الإضافة": "Added",
    "مهام مضافة بالفعل": "Already added missions",
    "مهام متاحة للإضافة": "Missions available to add",
    "اختر نوعاً آخر": "Choose another type",
    "يمكنك اختيار نوع آخر أو العودة إلى قائمة مهامك اليومية لإنجازها": "You can choose another type or go back to your daily list to complete your missions",

    // Roles
    "admin.role.Student": "Student",
    "admin.role.Teacher": "Teacher",
    "admin.role.Parent": "Parent",
    "admin.role.Admin": "Admin",

    // Verified status
    "admin.verified.yes": "Yes",
    "admin.verified.no": "No",

    // Search
    "admin.search.placeholder": "Search by name or email...",

    // Create button
    "admin.create.user": "Create New Account",
    "admin.create.class": "Create New Class",
    "admin.create.organization": "Create New School",
    "admin.create.grade": "Create New Grade",
    "admin.button.createGrade": "Create New Grade",

    // Create modal
    "admin.modal.createUser": "Create New Account",
    "admin.modal.accountType": "Account Type",
    "admin.modal.firstName": "First Name",
    "admin.modal.lastName": "Last Name",
    "admin.modal.email": "Email",
    "admin.modal.grade": "Stage",
    "admin.modal.selectGrade": "Select Stage",
    "admin.modal.org": "School",
    "admin.modal.selectOrg": "Select School",
    "admin.modal.gradeName": "Grade Name",
    "admin.modal.createGrade": "Create New Grade",
    "admin.modal.editGrade": "Edit Grade",
    "admin.modal.class": "Class (optional)",
    "admin.modal.selectClass": "Select Class",
    "admin.modal.noClassesAssigned": "No classes assigned yet",
    "admin.modal.allGrades": "All Grades",
    "admin.modal.addClass": "Add",
    "admin.modal.createBtn": "Create Account",
    "admin.modal.creating": "Creating...",

    // Create success
    "admin.created.title": "Account Created Successfully",
    "admin.created.email": "Email",
    "admin.created.tempPassword": "Temporary Password",
    "admin.created.shareNote":
      "Share these credentials with the user for first-time login",
    "admin.created.done": "Done",
    "admin.created.copied": "Copied!",

    // Edit modal
    "admin.modal.editUser": "Edit User",
    "admin.modal.editClass": "Edit Class",
    "admin.modal.editOrg": "Edit School",
    "admin.modal.createClass": "Create New Class",
    "admin.modal.createOrg": "Create New School",
    "admin.modal.className": "Class Name",
    "admin.modal.orgName": "School Name",
    "admin.modal.noClass": "No Class",
    "admin.modal.saveBtn": "Save Changes",
    "admin.modal.saving": "Saving...",
    "admin.modal.createNewBtn": "Create",

    // Delete confirmation
    "admin.delete.title": "Confirm Deletion",
    "admin.delete.message": "Are you sure you want to delete",
    "admin.delete.messageEnd": "? This action cannot be undone.",
    "admin.delete.confirm": "Delete",
    "admin.delete.cancel": "Cancel",

    // Impersonate
    "admin.impersonate.button": "Sign In as Student",
    "admin.impersonate.success": "Signed in as student successfully",
    "admin.impersonate.banner": "You are currently browsing as student: {{name}}",
    "admin.impersonate.return": "Return to Admin",

    // Reset password confirmation
    "admin.reset.title": "Reset Password",
    "admin.reset.message": "Are you sure you want to reset the password for",
    "admin.reset.messageEnd": "?",
    "admin.reset.confirm": "Reset Password",
    "admin.reset.cancel": "Cancel",

    // Pagination
    "admin.pagination.showing": "Showing",
    "admin.pagination.of": "of",
    "admin.pagination.results": "results",
    "admin.pagination.page": "Page",

    // Loading & empty states
    "admin.loading": "Loading data...",
    "admin.empty": "No results found",
    "admin.empty.description": "Try adjusting your search or filters",

    // Toast messages
    "admin.toast.saveSuccess": "Saved successfully",
    "admin.toast.createSuccess": "Created successfully",
    "admin.toast.deleteSuccess": "Deleted successfully",
    "admin.toast.saveFailed": "Could not save changes",
    "admin.toast.deleteFailed": "Could not delete",
    "admin.toast.loadFailed": "Could not load data",
    "admin.toast.resetSuccess": "Password reset to:",
    "admin.toast.resetFailed": "Could not reset password",
    "admin.toast.firstNameEmailRequired": "First name and email are required",
    "admin.toast.selectOrg": "Please select a school",
    "admin.toast.classNameGradeOrgRequired":
      "Class name, grade, and school are required",
    "admin.toast.orgNameRequired": "School name is required",
    "admin.toast.deleteHasRelated": "Cannot delete — related data exists:",
    "admin.toast.students": "students",
    "admin.toast.teachers": "teachers",
    "admin.toast.classes": "classes",

    // Grade values
    "admin.grade.primary": "Primary",
    "admin.grade.preparatory": "Preparatory",
    "admin.grade.secondary": "Secondary",

    // Bulk actions
    "admin.bulk.selected": "items selected",
    "admin.bulk.delete": "Delete Selected",
    "admin.bulk.resetPasswords": "Reset Passwords",
    "admin.bulk.deselectAll": "Deselect All",
    "admin.bulk.confirmDelete": "Delete selected items?",
    "admin.bulk.deleteSuccess": "Deleted {count} items",
    "admin.bulk.resetSuccess": "Passwords reset & CSV downloaded",

    // Filters
    "admin.filter.title": "Filters",
    "admin.filter.clear": "Clear All",
    "admin.filter.school": "School",
    "admin.filter.grade": "Grade",
    "admin.filter.role": "Role",
    "admin.filter.verified": "Verified Only",
    "admin.filter.all": "All",

    // Sorting
    "admin.sort.asc": "Sort ascending",
    "admin.sort.desc": "Sort descending",

    // Export / Import
    "admin.export.button": "Export CSV",
    "admin.export.toast": "CSV exported successfully",
    "admin.import.button": "Import",
    "admin.import.title": "Import from Spreadsheet",
    "admin.import.dropzone": "Drop CSV or Excel file here, or click to browse",
    "admin.import.preview": "Column Mapping",
    "admin.import.csvCol": "CSV Column",
    "admin.import.dbField": "Maps To",
    "admin.import.upload": "Upload",
    "admin.import.uploading": "Uploading...",
    "admin.import.success": "Import completed successfully",
    "admin.import.skip": "-- Skip --",
    "admin.import.reading": "Reading file...",
    "admin.import.downloadTemplate": "Download Template",
    "admin.import.downloadExample": "Download Example",
    "admin.import.officialDetected":
      "Official template detected — no mapping needed",
    "admin.import.ready": "Ready",
    "admin.import.warnings": "Warnings",
    "admin.import.blocked": "Blocked",
    "admin.import.status": "Status",
    "admin.import.ok": "OK",
    "admin.import.applySuggestion": "Apply",
    "admin.import.previewNote": "Showing first 10 rows of",
    "admin.import.startOver": "Start Over",
    "admin.import.importN": "Import {n} rows",
    "admin.import.importing": "Importing...",
    "admin.import.imported": "Imported",
    "admin.import.failed": "Failed",
    "admin.import.downloadErrors": "Download Failed Rows",
    "admin.import.done": "Done",

    // Analytics charts
    "admin.chart.distribution": "Role Distribution",
    "admin.chart.byGrade": "Students by Grade",
    "admin.chart.growth": "Signup Trend",

    // Quick-actions
    "admin.action.copyEmail": "Copy Email",
    "admin.action.copyCode": "Copy Connect Code",
    "admin.action.copied": "Copied!",

    // Misc
    "admin.gradeNameRequired": "Grade name is required",
    "admin.toast.gradeNameRequired": "Grade name is required",

    // Contextual guided onboarding
    "ابدأ من مهام اليوم": "Start with today's missions",
    "أهم شيء هنا: افتح التحديات اليومية وأنجز مهمة واحدة. باقي الصفحة يعرض مستواك وتقدم شجرتك.":
      "Most important: open today's challenges and finish one mission. The rest shows your level and tree progress.",
    "مهامك اليومية": "Your daily missions",
    "اختر مهمة، أنجزها، ثم اضغط تم الإنجاز لتحصل على المكافأة.":
      "Choose a mission, do it, then tap Completed to get the reward.",
    "ابدأ باختيار قسم واحد. بعد إنجاز المهمة أرسل طلب الموافقة.":
      "Start by choosing one category. After finishing the mission, send the approval request.",
    "المستوى ونقاط الخبرة": "Level and XP",
    "تكسب XP من المهام. عندما يرتفع مستواك تفتح أوسمة ومكافآت جديدة.":
      "You earn XP from missions. Higher levels unlock new medals and rewards.",
    "التقدم والشجرة": "Progress and tree",
    "هنا ترى نمو الشجرة ومواردك. استخدم المتجر فقط عندما تريد رعاية الشجرة.":
      "Here you see tree growth and resources. Use the store only when you want to care for the tree.",
    "اشتر الماء أو السماد عندما تحتاجهما لرعاية الشجرة.":
      "Buy water or fertilizer when you need them to care for the tree.",
    "هنا ترى ترتيبك بين زملائك وتستخدم البحث أو الفلاتر عند الحاجة.":
      "Here you see your ranking and use search or filters when needed.",
    "هذه الصفحة تعرض مستواك الحالي والأوسمة التي فتحتها أو اقتربت منها.":
      "This page shows your current level and medals you unlocked or are close to.",
    "صفحة ولي الأمر": "Parent home",
    "أهم شيء هنا: اربط طفلك وتابع طلبات الموافقة من الإشعارات.":
      "Most important: link your child and watch approval requests from notifications.",
    "صفحة المعلم": "Teacher home",
    "أهم شيء هنا: راقب الطلاب وراجع طلبات الموافقة على المهام.":
      "Most important: monitor students and review mission approval requests.",
    "لوحة المشرف": "Admin dashboard",
    "أهم شيء هنا: إدارة الطلاب والمعلمين واستيراد البيانات عند الحاجة.":
      "Most important: manage students and teachers, and import data when needed.",
    "مرحباً بك!": "Welcome!",
    "لنكمل مهمتك الأولى اليوم.": "Let's complete your first mission today.",
    "هنا تظهر مهامك اليومية": "This is where your daily missions appear",
    "اضغط هنا لعرض مهامك وإنجاز أول مهمة.":
      "Tap here to view your missions and complete your first one.",
    "أنجز مهمتك الأولى": "Complete your first mission",
    "اضغط على هذا الزر عند إتمام المهمة.":
      "Tap this button once you've completed the mission.",
    "رائع!": "Great!",
    "لقد حصلت على مكافأتك. استمر في إنجاز مهامك اليومية لكسب المزيد.":
      "You've earned your reward. Keep completing your daily missions to earn more.",
    "اختر نوع المهمة": "Choose a mission category",
    "اضغط على أحد الأقسام لعرض المهام.": "Tap a category to see its missions.",
    "اضغط لاختيار مهمتك.": "Tap to select your mission.",
    "اطلب الموافقة": "Request approval",
    "عند إنجاز المهمة، أرسلها لولي أمرك أو معلمك للموافقة عليها.":
      "Once you finish the mission, send it to your parent or teacher for approval.",
    "تم إرسال طلبك!": "Your request has been sent!",
    "سيقوم ولي أمرك أو معلمك بمراجعة طلبك قريبًا.":
      "Your parent or teacher will review your request soon.",
    "نقاط الخبرة (XP)": "Experience Points (XP)",
    "كل مهمة تنجزها تمنحك نقاط خبرة ترفع مستواك.":
      "Every mission you complete earns XP that raises your level.",
    "شجرتك تنمو!": "Your tree is growing!",
    "كل مرة تنجز فيها مهامك، تنمو شجرتك أكثر.":
      "Every time you complete your missions, your tree grows a little more.",
    "استخدم الماء والسماد لرعاية شجرتك من هنا.":
      "Use water and fertilizer to take care of your tree from here.",
    "شاهد ترتيبك بين زملائك هنا.":
      "See your ranking among your classmates here.",
    "الأوسمة والمستويات": "Medals & Levels",
    "كل مستوى تصل إليه يمنحك وسامًا جديدًا.":
      "Every level you reach earns you a new medal.",
    "اربط طفلك": "Link your child",
    "استخدم كود الدعوة لربط حساب طفلك بحسابك.":
      "Use the invite code to link your child's account to yours.",
    "ستصلك هنا طلبات موافقة المهام من أبنائك.":
      "Your children's mission approval requests will appear here.",
    "اعتماد المهام": "Approve missions",
    "يمكنك هنا قبول أو رفض طلبات إنجاز المهام.":
      "Here you can accept or decline mission completion requests.",
    "متابعة التقدم": "Track progress",
    "تابع إنجازات أبنائك وتقدمهم من هنا.":
      "Follow your children's achievements and progress from here.",
    "تم!": "Done!",
    "يمكنك دائمًا إعادة عرض هذه الجولة من الإعدادات.":
      "You can always replay this tour from Settings.",
    "لوحة التحكم": "Dashboard",
    "نظرة سريعة على طلابك وفصولك وتحدياتهم.":
      "A quick overview of your students, classes and challenges.",
    "تصفح وأدر طلابك من هنا.": "Browse and manage your students from here.",
    "طلبات المهام": "Mission requests",
    "ستصلك هنا طلبات موافقة مهام طلابك.":
      "Your students' mission approval requests will appear here.",
    المعلمون: "Teachers",
    "إدارة بيانات المعلمين من هنا.": "Manage teacher data from here.",
    "إدارة بيانات الطلاب من هنا.": "Manage student data from here.",
    "استيراد البيانات": "Import data",
    "استورد بيانات الطلاب أو المعلمين دفعة واحدة من ملف إكسل.":
      "Bulk-import student or teacher data from an Excel file.",
    التقارير: "Reports",
    "نظرة عامة على إحصائيات النظام.": "An overview of the system's statistics.",
    "جولة الصفحة الرئيسية": "Home tour",
    "جولة إنجاز المهام": "Completing missions tour",
    "جولة طلب الموافقة على المهام": "Mission approval request tour",
    "جولة نقاط الخبرة": "XP tour",
    "جولة الشجرة": "Tree tour",
    "جولة المتجر": "Shop tour",
    "جولة لوحة المتصدرين": "Leaderboard tour",
    "جولة الأوسمة": "Medals tour",
    "جولة الصفحة الرئيسية لولي الأمر": "Parent home tour",
    "جولة الصفحة الرئيسية للمعلم": "Teacher home tour",
    "جولة لوحة تحكم المشرف": "Admin dashboard tour",
    "أهلًا بك! 🎉": "Welcome! 🎉",
    "لنتعلم معًا كيف تنجز أول مهمة لك.":
      "Let's learn how to complete your first mission.",
    "هنا تظهر مهامك اليومية. اضغط عليها لتبدأ.":
      "Your daily missions appear here. Tap to start.",
    "أرسل إنجازك": "Send your work",
    "عند إنهاء المهمة اضغط هنا لإرسالها إلى معلمك أو ولي أمرك.":
      "When you finish a mission, tap here to send it to your teacher or parent.",
    "تم إرسال طلبك بنجاح! 🎉": "Your request has been sent successfully! 🎉",
    "أحسنت! ستحصل على مكافأتك بعد الموافقة.":
      "Well done! You'll get your reward once it's approved.",
    "أكمل مهمتك ✅": "Complete your task ✅",
    "عند إنهاء المهمة اضغط هنا لتحصل على مكافأتك فورًا.":
      "When you finish the task, tap here to get your reward instantly.",
    "أحسنت! حصلت على مكافأتك 🎉": "Well done! You earned your reward 🎉",
    "رائع! لقد حصلت على مكافأتك مباشرة. واصل إنجاز مهامك اليومية.":
      "Great! You got your reward right away. Keep completing your daily tasks.",
    "اضغط على العنصر المضيء للمتابعة": "Tap the highlighted item to continue",
    "ابدأ بربط حساب طفلك باستخدام كود الدعوة الخاص به.":
      "Start by linking your child's account using their invite code.",
    "طلبات الموافقة": "Approval requests",
    "الموافقات": "Approvals",
    "من الإشعارات تصلك طلبات إنجاز المهام لتوافق عليها.":
      "Mission approval requests arrive in your notifications.",
    "تابع طلابك": "Monitor your students",
    "من هنا تسجل إنجازات طلابك وتتابع تقدمهم.":
      "Record your students' achievements and follow their progress here.",
    "إدارة المستخدمين": "User management",
    "تنقّل بين الطلاب والمعلمين وأولياء الأمور من هنا.":
      "Switch between students, teachers, and parents here.",
    الاستيراد: "Import",
    "أضف دفعة كاملة من ملف إكسل بضغطة واحدة.":
      "Add a whole batch from an Excel file in one click.",
    "جولة إحصائيات المهام": "Mission stats tour",
    إحصائياتك: "Your stats",
    "تابع هنا عدد المهام التي أنجزتها يوميًا وأسبوعيًا.":
      "Track how many missions you complete daily and weekly here.",
    "جولة الكؤوس": "Trophies tour",
    "أكمل التحديات لتجمع الكؤوس وتفتح إنجازات جديدة.":
      "Complete challenges to collect trophies and unlock new achievements.",
    "لا توجد جوائز بعد": "No trophies yet",
    "أكمل التحديات لتحصل على جوائزك الأولى":
      "Complete challenges to earn your first trophy.",
    "جاري تحميل الجوائز...": "Loading trophies...",
    "جولة الملف الشخصي": "Profile tour",
    "ملفك الشخصي": "Your profile",
    "من هنا تشارك كود الربط مع ولي أمرك وتغيّر صورتك وإعداداتك.":
      "Share your link code with your parent and change your avatar and settings here.",
    "جولة الإشعارات": "Notifications tour",
    "تصلك هنا أخبار الكؤوس والإنجازات الجديدة.":
      "News about new trophies and achievements arrives here.",
    "جولة تسجيل الإنجازات": "Recording achievements tour",
    "تسجيل الإنجازات": "Record achievements",
    "اختر طالبًا ثم سجّل المهمة التي أنجزها.":
      "Pick a student, then record the mission they completed.",
    السابق: "Previous",
    "اضغط هنا للمتابعة": "Tap here to continue",
    إنهاء: "Finish",
    التالي: "Next",
    "صورتك الشخصية جاهزة!": "Your avatar is ready!",
    "لنبدأ رحلتك الآن.": "Let's start your journey now.",
    "إعادة عرض الجولات التعريفية": "Replay onboarding tours",
    "لا توجد جولات تعريفية متاحة بعد.": "No onboarding tours available yet.",
    "إعادة عرض الجولة التعريفية": "Replay onboarding tour",
    "مرحباً بك في سنابل الإحسان 🌱": "Welcome to Sanabel Al-Ehsan 🌱",
    "لتصلك تنبيهات مواعيد الصلاة وأعمال الخير اليومية بدقة، نرجو تفعيل الإشعارات والموقع الجغرافي.": "To accurately receive prayer alerts and daily good deeds reminders, please enable notifications and location.",
    "تنبيهات الصلاة والتحديات": "Prayer & Challenge Alerts",
    "تذكيرات مواقيت الصلوات الخمس والمهام": "Daily reminders for the five prayers and challenges",
    "تحديد الموقع التلقائي": "Automatic Location Detection",
    "حساب أوقات الصلاة الدقيقة لمدينتك": "Accurate prayer time calculations for your city",
    "تفعيل الإشعارات وتحديد الموقع": "Enable Notifications & Location",
    "اختيار مدينتي يدويًا": "Choose My City Manually",
    "اختر مدينتك يدويًا 📍": "Choose Your City Manually 📍",
    "اختر أقرب مدينة لك لحساب مواقيت الصلاة بدقة:": "Choose your nearest city for accurate prayer times:",
    "جارٍ تحديد موقعك وتفعيل التنبيهات...": "Detecting your location and enabling notifications...",
    "يرجى الموافقة على إذن الإشعارات والموقع": "Please allow notification and location permissions",
    "🔄 إعادة المحاولة تلقائيًا": "🔄 Try Auto-Detect Again",
    "تم الإعداد بنجاح! 🎉": "Setup Completed Successfully! 🎉",
    "تعذر تحديث بيانات الطالب": "Could not refresh student data",
    "تقليل السماد": "Decrease fertilizer",
    "زيادة السماد": "Increase fertilizer",
    "تقليل الماء": "Decrease water",
    "زيادة الماء": "Increase water",
    "يرجى تسجيل الدخول أولاً": "Please sign in first",
    "يرجى الانتظار حتى تكتمل العملية الحالية": "Please wait for the current action to finish",
    "تم تحديد مدينتك:": "Your city is set to:",
    "تم ضبط مواقيت الصلاة وتفعيل تنبيهات سنابل الإحسان.": "Prayer times configured and Sanabel notifications enabled.",
    "إعداد سريع وآمن": "Quick and secure setup",
    "خصوصيتك محفوظة. نستخدم موقعك فقط لحساب مواقيت الصلاة ولا نشاركه مع الآخرين.":
      "Your privacy is protected. We use your location only to calculate prayer times and never share it with others.",
    "اختيار يدوي": "Choose manually",
    "الموقع": "Location",
    "To Do": "To Do",
    "Added by you": "Added by you",
    "Assigned by": "Assigned by",
    "No prior assignment": "No prior assignment",
    "Waiting for approval": "Waiting for approval",
    "Completed": "Completed",
    "Confirmed by": "Confirmed by",
    "Choose a Parent or Teacher": "Choose a Parent or Teacher",
    "Assign Mission": "Assign Mission",
    "Register Completed Mission": "Register Completed Mission",
  },
};

export default en;
