...
const ClassList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void fetch(`${API_BASE_URL}/teachers/appear-Taskes-Type-Category/${selectedCategoryId}/${encodeURIComponent(selectedType)}`, {
  ...
  const addProgress = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/teachers/${endpoint}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (
          ...
          // — the teacher had no way to know the tap did nothing.
